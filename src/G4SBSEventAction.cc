...
void G4SBSEventAction::EndOfEventAction(const G4Event *evt)
{
  ...
  G4SBSNeuArmTOFSD *NeuArmTOFSDptr;
  G4SBSNeuArmTOFHitsCollection *NeuArmTOFHC = 0;

  G4SBSNeuArmVirtualSD *NeuArmVirtualSDptr;
  G4SBSNeuArmVirtualHitsCollection *NeuArmVirtualHC = 0;

  G4SBSCalHitsCollection *NCalHC = 0;
  ...
  for (set<G4String>::iterator d = SDlist.begin(); d != SDlist.end(); ++d)
  {
    ...
    G4SBSNeuArmTOFoutput neu_tof;
    G4SBSNeuArmVirtualoutput neu_vir;
    G4SBSNCALoutput ncal;
    ...
    case G4SBS::kAVFF_TOF:
      NeuArmTOFSDptr = (G4SBSNeuArmTOFSD *)SDman->FindSensitiveDetector(*d, false);
      if (NeuArmTOFSDptr != NULL)
      {
        NeuArmTOFHC = (G4SBSNeuArmTOFHitsCollection *)(HCE->GetHC(SDman->GetCollectionID(colNam = NeuArmTOFSDptr->GetCollectionName(0))));
        if (NeuArmTOFHC != NULL)
        {
    ...
    case G4SBS::kAVFF_Virtual:
      NeuArmVirtualSDptr = (G4SBSNeuArmVirtualSD *)SDman->FindSensitiveDetector(*d, false);
      if (NeuArmVirtualSDptr != NULL)
      {
        NeuArmVirtualHC = (G4SBSNeuArmVirtualHitsCollection *)HCE->GetHC(SDman->GetCollectionID(colNam = NeuArmVirtualSDptr->GetCollectionName(0)));
    ...
    case G4SBS::kAVFF_NCal:
      NCalSDptr = (G4SBSCalSD *)SDman->FindSensitiveDetector(*d, false);
      if (NCalSDptr != NULL)
      {
        NCalHC = (G4SBSCalHitsCollection *)(HCE->GetHC(SDman->GetCollectionID(colNam = NCalSDptr->GetCollectionName(0))));

        if (NCalHC != NULL)
        {
          ncal.timewindow = NCalSDptr->GetTimeWindow();
          ncal.threshold = NCalSDptr->GetEnergyThreshold();
          ncal.ntimebins = NCalSDptr->GetNTimeBins();
          ncal.hold_tbins = NCalSDptr->hold_tbins;

          sd = NCalSDptr->SDtracks;
          ...
          FillNCalData(evt, NCalHC, ncal, *sdtemp);

          fIO->SetNCalData(*d, ncal);

          anyhits = (anyhits || ncal.nhits_CAL > 0);
...
void G4SBSEventAction::FillGEMData(const G4Event *evt, G4SBSGEMHitsCollection *hits, G4SBSGEMoutput &gemoutput, G4SBSSDTrackOutput &sdtracks)
{
  ...
  for (G4int i = 0; i < hits->entries(); i++)
  {
    ...
    { // existing track in this layer, additional step; increment sums and averages:
      ...
      if ((*hits)[i]->GetHittime() < tmin[gemID][trid])
        tmin[gemID][trid] = (*hits)[i]->GetHittime();
      if ((*hits)[i]->GetHittime() > tmax[gemID][trid])
        tmax[gemID][trid] = (*hits)[i]->GetHittime();
      ...
      if ((*hits)[i]->GetMom() < pmin[gemID][trid])
      { // or rather that ???
  ...
  for (map<int, set<int>>::iterator hit = tracks_layers.begin(); hit != tracks_layers.end(); hit++)
  {
    ...
    for (set<int>::iterator track = tracklist.begin(); track != tracklist.end(); track++)
    {
      ...
      if (edep[gemID][trackID] >= gemoutput.threshold)
      {
        ...
        if (trajectorylist)
        { // Fill Particle History, starting with the particle itself and working all the way back to primary particles:
          ...
          do
          {
            ...
            { // This trajectory does not yet exist in the particle history of this detector for this event. Add it:
...
void G4SBSEventAction::FillCalData(const G4Event *evt, G4SBSCalHitsCollection *hits, G4SBSCALoutput &caloutput, G4SBSSDTrackOutput &SDtracks)
{
  ...
  for (G4int hit = 0; hit < hits->entries(); hit++)
  {
    ...
    if (pid != 0 && Edep > 0.0)
    { // exclude optical photons and other non-physical particles
      // Global hit information:
      if (newcell.second)
      { // first hit in a new cell:
        nsteps_cell[cell] = 1;
        // nhits_cell[cell] = 1;
      ...
      { // additional step in existing cell:
      ...
      { // additional step in this cell:
        ...
        z[cell][track] += Edep * (*hits)[hit]->GetPos().z();
        trt[cell][track] += Edep * (*hits)[hit]->GetTime();
        if ((*hits)[hit]->GetTime() < trtmin[cell][track])
          trtmin[cell][track] = (*hits)[hit]->GetTime();
        if ((*hits)[hit]->GetTime() > trtmax[cell][track])
          trtmax[cell][track] = (*hits)[hit]->GetTime();
  ...
  for (set<int>::iterator itcell = CellList.begin(); itcell != CellList.end(); itcell++)
  {
    ...
    for (int istep = 0; istep < steplist.size(); istep++)
    {
      ...
      { // Add this step to the current hit:
        ...
        if (bin_tstep >= 0 && bin_tstep < caloutput.ntimebins)
          esum_tbin[cell][hitindex][bin_tstep] += estep;
        // ******
    ...
    for (set<int>::iterator itrack = TrackIDs[cell].begin(); itrack != TrackIDs[cell].end(); itrack++)
    {
      ...
      if (goodhit_index.find(firsthit_track[track]) != goodhit_index.end())
      { // Track must be associated with at least one "GOOD" hit in this cell to be recorded:
...
void G4SBSEventAction::FillECalData(G4SBSECalHitsCollection *hits, G4SBSECaloutput &ecaloutput, G4SBSSDTrackOutput &SDtracks)
{
  ...
  for (int step = 0; step < nG4hits; step++)
  {
    ...
    if (newpmt.second)
    { // first hit in a new pmt:

      if (photon_det)
      { // I assume we only care about detected photons in this case
        nsteps_pmt[pmt_un] = 1;
        // PmtList.insert( pmt );
    ...
    { // additional hits in existing PMT
    ...
    { // existing photon, additional step. Increment averages of position, direction, time, etc for all steps of a detected photon. Don't bother for
      // undetected photons...
  ...
  while (remaining_hits)
  {
    ...
    for (set<int>::iterator it = TIDs_unique.begin(); it != TIDs_unique.end(); it++)
    {
      ...
      if (Photon_detected[tid] && !(Photon_used[tid]))
      {
        ...
        else if (fabs(Photon_hittime[tid] - PMT_tmin[pmt]) <= ecaloutput.timewindow)
        { // Existing pmt with multiple photon detections:
          ...
          if (Photon_hittime[tid] > PMT_tmax[pmt])
            PMT_tmax[pmt] = Photon_hittime[tid];
          if (Photon_hittime[tid] < PMT_tmin[pmt])
            PMT_tmin[pmt] = Photon_hittime[tid];
    ...
    for (set<int>::iterator it = PMTs_unique.begin(); it != PMTs_unique.end(); it++)
    {
      ...
      for (int istep = 0; istep < steplist.size(); istep++)
      {
        ...
        else
        {
          ...
          if (bin_tstep >= 0 && bin_tstep < ecaloutput.ntimebins)
            npe_tbin[pmt][hitindex][bin_tstep] += 1.;
        }
      }
...
void G4SBSEventAction::FillRICHData(const G4Event *evt, G4SBSRICHHitsCollection *hits, G4SBSRICHoutput &richoutput, G4SBSSDTrackOutput &SDtracks)
{
  ...
  for (int step = 0; step < nG4hits; step++)
  {
    ...
    if (testphotontrack.second)
    { // New photon track: determine whether this photon is detected:
  ...
  while (remaining_hits)
  {
    ...
    for (set<int>::iterator it = TIDs_unique.begin(); it != TIDs_unique.end(); it++)
    {
      ...
      if (Photon_detected[tid] && !(Photon_used[tid]))
      {
        ...
        else if (fabs(Photon_hittime[tid] - PMT_hittime[pmt]) <= richoutput.timewindow)
        { // Existing pmt with multiple photon detections:
  ...
  { // this is a loop over all TIDs that produced photoelectrons:
    ...
    do
    {
      ...
      if (newtrajectory.second)
      {
        ...
        if (mTIDs_unique.find(TIDtemp) != mTIDs_unique.end())
        { // if this track is in the set of all tracks producing photodetections, record the number of photoelectrons associated:
...
void G4SBSEventAction::FillTrackData(G4SBSGEMoutput gemdata, G4SBSTrackerOutput &Toutput)
{
  ...
  for (set<int>::iterator trk = TrackTIDs_unique.begin(); trk != TrackTIDs_unique.end(); trk++)
  {
    ...
    if (nhittrk >= nplanes_min)
    {
      ...
      { // First pass: determine number of unique GEM planes and compute average slopes of "true track". Also compute the sums for the linear fit:
...
void G4SBSEventAction::FillBDData(const G4Event *evt, G4SBSBDHitsCollection *hc, G4SBSBDoutput &out)
{
  ...
  for (int i = 0; i < NHits; i++)
  {
    ...
    if (debug)
    {
      ...
      if ((i + 1) < 5)
        std::cout << msg << std::endl; // print first 5 hits
    }
  }
...
void G4SBSEventAction::FillNeuArmTOFData(G4SBSNeuArmTOFHitsCollection *hc, G4SBSNeuArmTOFoutput &out)
{
  out.Clear();
  ...
  for (int i = 0; i < nHits; ++i)
  {
    G4SBSNeuArmTOFHit *hit = (*hc)[i];
    out.fPID.push_back(hit->GetPdgID());
    out.fTID.push_back(hit->GetTrackID());
    out.fPTID.push_back(hit->GetParentTrackID());
    out.fDID.push_back(hit->GetDetectorID());
    out.fProcessID.push_back(hit->GetProcessID());
    out.fX.push_back(hit->GetInXYZ().getX());
    out.fY.push_back(hit->GetInXYZ().getY());
    out.fZ.push_back(hit->GetInXYZ().getZ());
    out.fPx.push_back(hit->GetInPxPyPz().getX());
    out.fPy.push_back(hit->GetInPxPyPz().getY());
    out.fPz.push_back(hit->GetInPxPyPz().getZ());
    out.fOutX.push_back(hit->GetOutXYZ().getX());
    out.fOutY.push_back(hit->GetOutXYZ().getY());
    out.fOutZ.push_back(hit->GetOutXYZ().getZ());
    out.fVx.push_back(hit->GetVertexXYZ().getX());
...
void G4SBSEventAction::FillNeuArmVirtualData(G4SBSNeuArmVirtualHitsCollection *hc, G4SBSNeuArmVirtualoutput &out)
{
  out.Clear();
  G4int nHits = hc->entries();
  for (int i = 0; i < nHits; ++i)
  {
    G4SBSNeuArmVirtualHit *hit = (*hc)[i];
    out.fPID.push_back(hit->GetPdgID());
    out.fTID.push_back(hit->GetTrackID());
    out.fPTID.push_back(hit->GetParentTrackID());
    out.fDID.push_back(hit->GetDetectorID());
...
void G4SBSEventAction::FillNCalData(const G4Event *evt, G4SBSCalHitsCollection *hits, G4SBSNCALoutput &caloutput, G4SBSSDTrackOutput &SDtracks)
{
  // The "CAL" output class provides two kinds of information:
  // 1. Sum of energy deposition in a cell.
  // 2. List of all particles in a cell with track ids, pids, mids, coordinates, vertices, etc. (also momentum and energy deposition)
