...
G4SBSIO::~G4SBSIO()
{
  ...
  if (Esum_histograms)
  {
    delete Esum_histograms;
  }
  if (PulseShape_histograms)
  {
    delete PulseShape_histograms;
  }
}
void G4SBSIO::SetNeuArmVirtualData(G4String SDname, G4SBSNeuArmVirtualoutput vird)
{
  NeuArmVirtualData[SDname] = vird;
}
void G4SBSIO::SetNeuArmTOFData(G4String SDname, G4SBSNeuArmTOFoutput tofd)
{
  NeuArmTOFData[SDname] = tofd;
}
void G4SBSIO::SetNCalData(G4String SDname, G4SBSNCALoutput ncald)
{
  NCalData[SDname] = ncald;
}
...
void G4SBSIO::InitializeTree()
{
  ...
  { // Create branches for Collins and Sivers asymmetries (eventually others, like quark flavor contributions to cross sections/asymmetries/etc)
  ...
  for (set<G4String>::iterator d = (fdetcon->SDlist).begin(); d != (fdetcon->SDlist).end(); d++)
  {
    ...
    case G4SBS::kRICH: //"RICH"
      ...
      if (fUsingCerenkov || fUsingScintillation)
        BranchRICH(SDname);
      break;
    ...
    case G4SBS::kTarget_GEn_3He:
      ...
      break;
    case G4SBS::kAVFF_TOF:
      NeuArmTOFData[SDname] = G4SBSNeuArmTOFoutput();
      BranchNeuArmTOF(SDname);
      break;
    case G4SBS::kAVFF_Virtual:
      NeuArmVirtualData[SDname] = G4SBSNeuArmVirtualoutput();
      BranchNeuArmVirtual(SDname);
      break;
    case G4SBS::kAVFF_NCal:
      NCalData[SDname] = G4SBSNCALoutput();
      BranchNCal(SDname);
      break;
      }
...
void G4SBSIO::WriteTree()
{
  ...
  if (fdetcon->fUseGlobalField && fWritePortableFieldMaps)
  { // We are using a global field definition with "TOSCA" maps, and the user wants to write portable ones based on a subset of the global field volume:
    bool writeSBS = false;
    bool writeBB = false;
    ...
    writeBB = exp != G4SBS::kGEp && exp != G4SBS::kC16 && exp != G4SBS::kTDIS && exp != G4SBS::kNDVCS && exp != G4SBS::kGEMHCtest && exp != G4SBS::kGEPpositron; // BigBite is used in all experiments except C16, gem test, TDIS, NDVCS, and GEP positron
...
void G4SBSIO::BranchAVFFGen()
{
  fTree->Branch("GenPart.Weight", &(fAVFFGen.fWeight), "GenPart.Weight/F");
  fTree->Branch("GenPart.X", &(fAVFFGen.fVx), "GenPart.X/F");
  fTree->Branch("GenPart.Y", &(fAVFFGen.fVy), "GenPart.Y/F");
  fTree->Branch("GenPart.Z", &(fAVFFGen.fVz), "GenPart.Z/F");
  fTree->Branch("GenPart.PDGID", &(fAVFFGen.fPdgID));
...
void G4SBSIO::UpdateGenDataFromDetCon()
{ // Go with whatever is in fdetcon as of run start for constant parameters of the run describing detector layout:
...
void G4SBSIO::BranchNeuArmTOF(G4String SDname)
{
  TString branch_name;
  TString branch_prefix = SDname.data();
  branch_prefix.ReplaceAll("/", ".");
  fTree->Branch(branch_name.Format("%s.PDGID", branch_prefix.Data()), &(NeuArmTOFData[SDname].fPID));
  fTree->Branch(branch_name.Format("%s.TrackID", branch_prefix.Data()), &(NeuArmTOFData[SDname].fTID));
  fTree->Branch(branch_name.Format("%s.ParentTrackID", branch_prefix.Data()), &(NeuArmTOFData[SDname].fPTID));
  fTree->Branch(branch_name.Format("%s.DetID", branch_prefix.Data()), &(NeuArmTOFData[SDname].fDID));
  fTree->Branch(branch_name.Format("%s.ProcessID", branch_prefix.Data()), &(NeuArmTOFData[SDname].fProcessID));
  fTree->Branch(branch_name.Format("%s.HitInPosition.X", branch_prefix.Data()), &(NeuArmTOFData[SDname].fX));
  fTree->Branch(branch_name.Format("%s.HitInPosition.Y", branch_prefix.Data()), &(NeuArmTOFData[SDname].fY));
  fTree->Branch(branch_name.Format("%s.HitInPosition.Z", branch_prefix.Data()), &(NeuArmTOFData[SDname].fZ));
  fTree->Branch(branch_name.Format("%s.HitInMomentum.X", branch_prefix.Data()), &(NeuArmTOFData[SDname].fPx));
  fTree->Branch(branch_name.Format("%s.HitInMomentum.Y", branch_prefix.Data()), &(NeuArmTOFData[SDname].fPy));
  fTree->Branch(branch_name.Format("%s.HitInMomentum.Z", branch_prefix.Data()), &(NeuArmTOFData[SDname].fPz));
  fTree->Branch(branch_name.Format("%s.HitVertexPosition.X", branch_prefix.Data()), &(NeuArmTOFData[SDname].fVx));
  fTree->Branch(branch_name.Format("%s.HitVertexPosition.Y", branch_prefix.Data()), &(NeuArmTOFData[SDname].fVy));
  fTree->Branch(branch_name.Format("%s.HitVertexPosition.Z", branch_prefix.Data()), &(NeuArmTOFData[SDname].fVz));
  fTree->Branch(branch_name.Format("%s.HitOutPosition.X", branch_prefix.Data()), &(NeuArmTOFData[SDname].fOutX));
  fTree->Branch(branch_name.Format("%s.HitOutPosition.Y", branch_prefix.Data()), &(NeuArmTOFData[SDname].fOutY));
  fTree->Branch(branch_name.Format("%s.HitOutPosition.Z", branch_prefix.Data()), &(NeuArmTOFData[SDname].fOutZ));
...
void G4SBSIO::BranchNeuArmVirtual(G4String SDname)
{
  TString branch_name;
  TString branch_prefix = SDname.data();
  branch_prefix.ReplaceAll("/", ".");
  G4SBSNeuArmVirtualSD *SDtemp = (G4SBSNeuArmVirtualSD *)fdetcon->fSDman->FindSensitiveDetector(SDname);
  fTree->Branch(branch_name.Format("%s.PDGID", branch_prefix.Data()), &(NeuArmVirtualData[SDname].fPID));
  fTree->Branch(branch_name.Format("%s.TrackID", branch_prefix.Data()), &(NeuArmVirtualData[SDname].fTID));
  fTree->Branch(branch_name.Format("%s.ParentTrackID", branch_prefix.Data()), &(NeuArmVirtualData[SDname].fPTID));
  fTree->Branch(branch_name.Format("%s.DetID", branch_prefix.Data()), &(NeuArmVirtualData[SDname].fDID));
  fTree->Branch(branch_name.Format("%s.HitInPosition.X", branch_prefix.Data()), &(NeuArmVirtualData[SDname].fX));
  fTree->Branch(branch_name.Format("%s.HitInPosition.Y", branch_prefix.Data()), &(NeuArmVirtualData[SDname].fY));
  fTree->Branch(branch_name.Format("%s.HitInPosition.Z", branch_prefix.Data()), &(NeuArmVirtualData[SDname].fZ));
  fTree->Branch(branch_name.Format("%s.HitInMomentum.X", branch_prefix.Data()), &(NeuArmVirtualData[SDname].fPx));
  fTree->Branch(branch_name.Format("%s.HitInMomentum.Y", branch_prefix.Data()), &(NeuArmVirtualData[SDname].fPy));
  fTree->Branch(branch_name.Format("%s.HitInMomentum.Z", branch_prefix.Data()), &(NeuArmVirtualData[SDname].fPz));
  fTree->Branch(branch_name.Format("%s.VertexPosition.X", branch_prefix.Data()), &(NeuArmVirtualData[SDname].fVx));
  fTree->Branch(branch_name.Format("%s.VertexPosition.Y", branch_prefix.Data()), &(NeuArmVirtualData[SDname].fVy));
  fTree->Branch(branch_name.Format("%s.VertexPosition.Z", branch_prefix.Data()), &(NeuArmVirtualData[SDname].fVz));
  fTree->Branch(branch_name.Format("%s.HitGlobalTime", branch_prefix.Data()), &(NeuArmVirtualData[SDname].fTime));
}

void G4SBSIO::BranchNCal(G4String SDname)
{
  TString branch_prefix = SDname.data();
  TString branch_name;
  ...
  // Define "hit" branches:
  fTree->Branch(branch_name.Format("%s.det.esum", branch_prefix.Data()), &(NCalData[SDname].Esum));
  fTree->Branch(branch_name.Format("%s.hit.nhits", branch_prefix.Data()), &(NCalData[SDname].nhits_CAL));
  fTree->Branch(branch_name.Format("%s.hit.row", branch_prefix.Data()), &(NCalData[SDname].row));
  fTree->Branch(branch_name.Format("%s.hit.col", branch_prefix.Data()), &(NCalData[SDname].col));
  fTree->Branch(branch_name.Format("%s.hit.cell", branch_prefix.Data()), &(NCalData[SDname].cell));
  fTree->Branch(branch_name.Format("%s.hit.plane", branch_prefix.Data()), &(NCalData[SDname].plane));
  fTree->Branch(branch_name.Format("%s.hit.wire", branch_prefix.Data()), &(NCalData[SDname].wire));
  fTree->Branch(branch_name.Format("%s.hit.xcell", branch_prefix.Data()), &(NCalData[SDname].xcell));
  fTree->Branch(branch_name.Format("%s.hit.ycell", branch_prefix.Data()), &(NCalData[SDname].ycell));
  fTree->Branch(branch_name.Format("%s.hit.zcell", branch_prefix.Data()), &(NCalData[SDname].zcell));
  fTree->Branch(branch_name.Format("%s.hit.xcellg", branch_prefix.Data()), &(NCalData[SDname].xcellg));
  fTree->Branch(branch_name.Format("%s.hit.ycellg", branch_prefix.Data()), &(NCalData[SDname].ycellg));
  fTree->Branch(branch_name.Format("%s.hit.zcellg", branch_prefix.Data()), &(NCalData[SDname].zcellg));
  fTree->Branch(branch_name.Format("%s.hit.xhit", branch_prefix.Data()), &(NCalData[SDname].xhit));
  fTree->Branch(branch_name.Format("%s.hit.yhit", branch_prefix.Data()), &(NCalData[SDname].yhit));
  fTree->Branch(branch_name.Format("%s.hit.zhit", branch_prefix.Data()), &(NCalData[SDname].zhit));
  fTree->Branch(branch_name.Format("%s.hit.xhitg", branch_prefix.Data()), &(NCalData[SDname].xhitg));
  fTree->Branch(branch_name.Format("%s.hit.yhitg", branch_prefix.Data()), &(NCalData[SDname].yhitg));
  fTree->Branch(branch_name.Format("%s.hit.zhitg", branch_prefix.Data()), &(NCalData[SDname].zhitg));
  fTree->Branch(branch_name.Format("%s.hit.sumedep", branch_prefix.Data()), &(NCalData[SDname].sumedep));
  fTree->Branch(branch_name.Format("%s.hit.tavg", branch_prefix.Data()), &(NCalData[SDname].tavg));
  fTree->Branch(branch_name.Format("%s.hit.trms", branch_prefix.Data()), &(NCalData[SDname].trms));
  fTree->Branch(branch_name.Format("%s.hit.tmin", branch_prefix.Data()), &(NCalData[SDname].tmin));
  fTree->Branch(branch_name.Format("%s.hit.tmax", branch_prefix.Data()), &(NCalData[SDname].tmax));
  ...
  if (fKeepAllPulseShape || (keeppsflag != fKeepPulseShape.end() && keeppsflag->second))
  {
    fTree->Branch(branch_name.Format("%s.gatewidth", branch_prefix.Data()), &(NCalData[SDname].gatewidth));
    fTree->Branch(branch_name.Format("%s.hit.edep_vs_time", branch_prefix.Data()), &(NCalData[SDname].edep_vs_time));
  }
  ...
  {
    // Add "SD track" indices:
    fTree->Branch(branch_name.Format("%s.hit.otridx", branch_prefix.Data()), &(NCalData[SDname].otridx));
    fTree->Branch(branch_name.Format("%s.hit.ptridx", branch_prefix.Data()), &(NCalData[SDname].ptridx));
    fTree->Branch(branch_name.Format("%s.hit.sdtridx", branch_prefix.Data()), &(NCalData[SDname].sdtridx));
  }
  ...
  {
    // Define "particle" branches:
    fTree->Branch(branch_name.Format("%s.npart_NCAL", branch_prefix.Data()), &(NCalData[SDname].npart_CAL));
    fTree->Branch(branch_name.Format("%s.ihit", branch_prefix.Data()), &(NCalData[SDname].ihit));
    fTree->Branch(branch_name.Format("%s.x", branch_prefix.Data()), &(NCalData[SDname].x));
    fTree->Branch(branch_name.Format("%s.y", branch_prefix.Data()), &(NCalData[SDname].y));
    fTree->Branch(branch_name.Format("%s.z", branch_prefix.Data()), &(NCalData[SDname].z));
    fTree->Branch(branch_name.Format("%s.t", branch_prefix.Data()), &(NCalData[SDname].t));
    fTree->Branch(branch_name.Format("%s.E", branch_prefix.Data()), &(NCalData[SDname].E));
    fTree->Branch(branch_name.Format("%s.dt", branch_prefix.Data()), &(NCalData[SDname].dt));
    fTree->Branch(branch_name.Format("%s.L", branch_prefix.Data()), &(NCalData[SDname].L));
    fTree->Branch(branch_name.Format("%s.vx", branch_prefix.Data()), &(NCalData[SDname].vx));
    fTree->Branch(branch_name.Format("%s.vy", branch_prefix.Data()), &(NCalData[SDname].vy));
    fTree->Branch(branch_name.Format("%s.vz", branch_prefix.Data()), &(NCalData[SDname].vz));
    fTree->Branch(branch_name.Format("%s.trid", branch_prefix.Data()), &(NCalData[SDname].trid));
    fTree->Branch(branch_name.Format("%s.mid", branch_prefix.Data()), &(NCalData[SDname].mid));
    fTree->Branch(branch_name.Format("%s.pid", branch_prefix.Data()), &(NCalData[SDname].pid));
    fTree->Branch(branch_name.Format("%s.p", branch_prefix.Data()), &(NCalData[SDname].p));
    fTree->Branch(branch_name.Format("%s.px", branch_prefix.Data()), &(NCalData[SDname].px));
    fTree->Branch(branch_name.Format("%s.py", branch_prefix.Data()), &(NCalData[SDname].py));
    fTree->Branch(branch_name.Format("%s.pz", branch_prefix.Data()), &(NCalData[SDname].pz));
    fTree->Branch(branch_name.Format("%s.edep", branch_prefix.Data()), &(NCalData[SDname].edep));
  }
  ...
  {
    // Branches with "Particle History" data:
    fTree->Branch(branch_name.Format("%s.part.npart", branch_prefix.Data()), &(NCalData[SDname].ParticleHistory.npart));
    fTree->Branch(branch_name.Format("%s.part.PID", branch_prefix.Data()), &(NCalData[SDname].ParticleHistory.PID));
    fTree->Branch(branch_name.Format("%s.part.MID", branch_prefix.Data()), &(NCalData[SDname].ParticleHistory.MID));
    fTree->Branch(branch_name.Format("%s.part.TID", branch_prefix.Data()), &(NCalData[SDname].ParticleHistory.TID));
    fTree->Branch(branch_name.Format("%s.part.nbounce", branch_prefix.Data()), &(NCalData[SDname].ParticleHistory.nbounce));
    fTree->Branch(branch_name.Format("%s.part.hitindex", branch_prefix.Data()), &(NCalData[SDname].ParticleHistory.hitindex));
    fTree->Branch(branch_name.Format("%s.part.vx", branch_prefix.Data()), &(NCalData[SDname].ParticleHistory.vx));
    fTree->Branch(branch_name.Format("%s.part.vy", branch_prefix.Data()), &(NCalData[SDname].ParticleHistory.vy));
    fTree->Branch(branch_name.Format("%s.part.vz", branch_prefix.Data()), &(NCalData[SDname].ParticleHistory.vz));
    fTree->Branch(branch_name.Format("%s.part.px", branch_prefix.Data()), &(NCalData[SDname].ParticleHistory.px));
    fTree->Branch(branch_name.Format("%s.part.py", branch_prefix.Data()), &(NCalData[SDname].ParticleHistory.py));
    fTree->Branch(branch_name.Format("%s.part.pz", branch_prefix.Data()), &(NCalData[SDname].ParticleHistory.pz));
  }
 //*/
  return;
}
