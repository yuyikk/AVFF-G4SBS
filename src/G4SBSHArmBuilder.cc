...
void G4SBSHArmBuilder::BuildComponent(G4LogicalVolume *worldlog)
{
  ...
  else if (exptype == G4SBS::kGEp || exptype == G4SBS::kGEPpositron || exptype == G4SBS::kGEp_BB || exptype == G4SBS::kAxialFF)
  {
...
void G4SBSHArmBuilder::Make48D48(G4LogicalVolume *worldlog, double r48d48)
{
  ...
  double sign = 1.0;
  if (fDetCon->fGlobalField->fInverted)
    sign = -1.0;
  ...
  G4UniformMagField *magField = new G4UniformMagField(G4ThreeVector(sign * FieldMag * cos(f48D48ang), 0.0, sign * FieldMag * sin(f48D48ang)));
  ...
  if (fDetCon->fExpType == G4SBS::kGEp || fDetCon->fExpType == G4SBS::kGEPpositron)
  {
    ...
    G4VisAttributes *slabVisAtt = new G4VisAttributes(G4Colour(1.0, 0.1, 0.0));
...
void G4SBSHArmBuilder::MakeSBSFieldClamps(G4LogicalVolume *motherlog)
{
  ...
  if (false)
  { // BigBite: Field clamp version 1 (AFAIK) is obsolete. Never build this...
  ...
  else if (f48D48_fieldclamp_config != 0)
  { // 2 == GEp, 3 == GMn
    ...
    if (f48D48_fieldclamp_config == 2)
    { // GEp uses a back clamp too
      ////REAR CLAMP:
                                  ...
                                  G4double VerticalOffset)
{
  ...
  G4UnionSolid *sol_FrontPl0 = new G4UnionSolid(
      ...
      sol_EndPl0, sol_EndPl1, 0, G4ThreeVector(0., 0., (dim_EndPl0Z + dim_EndPl1Z) / 2.));
  ...
  G4UnionSolid *sol_LightGuide = new G4UnionSolid("sol_LightGuide",
                                                  sol_LightGuide0, sol_LightGuide1, 0, G4ThreeVector(0., 0., (dim_LightGuide0Z + dim_LightGuide1Z) / 2.));
  ...
  new G4PVPlacement(0, G4ThreeVector(0., (dim_WaveShiftY + dim_WaveShiftRodY) / 2., posZ), log_WaveShiftRod, "pWaveShiftRod1",
                    log_Module, false, 0, checkOverlap);
  new G4PVPlacement(0, G4ThreeVector(0., -(dim_WaveShiftY + dim_WaveShiftRodY) / 2., posZ), log_WaveShiftRod, "pWaveShiftRod1",
                    log_Module, false, 0, checkOverlap);
  posZ += dim_WaveShiftZ / 2;
...
void G4SBSHArmBuilder::MakeHCAL(G4LogicalVolume *motherlog, G4double VerticalOffset = 0.0 * cm)
{
  ...
  for (int ii = 0; ii < NumberOfLayers; ii++)
  {
    ...
    Zpos = (-ModuleLtotal / 2 + ContainerThick + IronPlThick + ScinPlThick / 2.0 + ii * (IronPlThick + ScinPlThick)) + scin_gap;
...
void G4SBSHArmBuilder::MakeElectronModeSBS(G4LogicalVolume *motherlog)
{
  ...
  G4SBS::Exp_t exptype = fDetCon->fExpType;
  if (exptype == G4SBS::kA1n)
  { // HCAL AND LAC:
    MakeHCALV2(motherlog, fHCALvertical_offset);
    MakeLAC(motherlog);
  }
  else
  { // TDIS or NDVCS: LAC only:
    MakeLAC(motherlog);
  }
}
...
void G4SBSHArmBuilder::MakeRICH_new(G4LogicalVolume *motherlog, bool extended_snout)
{
  ...
  if (extended_snout)
    RICHbox_thick += fRichSnoutExtension;
  // RICHbox_h = 10.0*m;
  ...
  if (extended_snout)
    origin.setZ(origin.z() + fRichSnoutExtension);
  ...
  if (extended_snout)
    frontframe_pos.setZ(frontframe_pos.z() - fRichSnoutExtension);
  ...
  if (extended_snout)
    frontwindow_pos.setZ(frontwindow_pos.z() - fRichSnoutExtension);
  ...
  if (extended_snout)
    z0_RICH = z0_RICH - fRichSnoutExtension;
...
void G4SBSHArmBuilder::MakeFPP(G4LogicalVolume *Mother, G4RotationMatrix *rot, G4ThreeVector pos)
{
  ...
  switch (fGEPFPPoption)
  { // 1 = 8-plane front tracker of 6x(40x150 cm^2)+2x(60x200 cm^2) and 8-plane back tracker of 6x(60x200 cm^2)
  case 1:
  default:
    nana = 1;
    ntracker = 2;
...
void G4SBSHArmBuilder::MakePolarimeterGEnRP(G4LogicalVolume *worldlog)
{
  ...
  if (fDetCon->fExpType != G4SBS::kGEN)
  {
    ...
    { // long axis parallel to neutron direction
    ...
    for (int ix = 0; ix < nactanabarsx; ix++)
    {
      ...
      for (int iz = 0; iz < nactanabarsz; iz++)
      {
        ...
        else if (fGEnRP_analyzer_option == 4)
        { // CGEN
