...
void G4SBSEArmBuilder::BuildComponent(G4LogicalVolume *worldlog)
{
  ...
  if ((exptype == G4SBS::kGMN || exptype == G4SBS::kGEN || exptype == G4SBS::kGEnRP) && fBuildGEMfrontend)
    MakeGMnGEMShielding(worldlog);
...
void G4SBSEArmBuilder::MakeBigBite(G4LogicalVolume *worldlog)
{
  ...
  G4LogicalVolume *bbdetLog = new G4LogicalVolume(bbdetbox, GetMaterial("Air"),
                                                  "bbdetLog", 0, 0, 0);
  new G4PVPlacement(bbdetrot, G4ThreeVector(0.0, (detboxplace + detboxdepth / 2.0) * sin(detboxang), (detboxplace + detboxdepth / 2.0) * cos(detboxang) + midplanez),
  ...
  */
  ...
  bbcal_mother_log->SetVisAttributes(G4VisAttributes::GetInvisible());
  ...
  //  bbcal_shield_log->SetVisAttributes( G4VisAttributes::GetInvisible() );
  ...
  bbhodolog->SetVisAttributes(G4VisAttributes::GetInvisible());
  ...
  */
  ...
  // Mother volumes
  bbdetLog->SetVisAttributes(G4VisAttributes::GetInvisible());
  bbfieldLog->SetVisAttributes(G4VisAttributes::GetInvisible());
  bbmotherLog->SetVisAttributes(G4VisAttributes::GetInvisible());

  bbpslog->SetVisAttributes(G4VisAttributes::GetInvisible());
...
void G4SBSEArmBuilder::MakeDVCSECal(G4LogicalVolume *motherlog)
{
  ...
  dvcsblkmodlog->SetVisAttributes(DVCSblkecalbox_visatt); // G4VisAttributes::GetInvisible() );//
...
void G4SBSEArmBuilder::MakeC16(G4LogicalVolume *motherlog)
{
  ...
  { // Default case: room temperature, no radiation damage:
    // Make a C16 Module
    ...
    for (int sub_row = 0; sub_row < 4; sub_row++)
    {
      ...
      if (nused42 + ncol42_row[super_row] <= nblocks_42)
      {
        ...
        for (int col = 0; col < ncol; col++)
        {
          ...
          if (col == 0)
            xlow_row = modpos.x() - 0.5 * width_42;
          if (col + 1 == ncol)
            xhigh_row = modpos.x() + 0.5 * width_42;
      ...
      else if (nused40 + ncol40_row[super_row] <= nblocks_40)
      {
        ...
        for (int col = 0; col < ncol; col++)
        {
          ...
          if (col == 0)
            xlow_row = modpos.x() - 0.5 * width_40;
          if (col + 1 == ncol)
            xhigh_row = modpos.x() + 0.5 * width_40;
      ...
      else if (nused38 + ncol38_row[super_row] <= nblocks_38)
      {
        ...
        for (int col = 0; col < ncol; col++)
        {
          ...
          if (col == 0)
            xlow_row = modpos.x() - 0.5 * width_38;
          if (col + 1 == ncol)
            xhigh_row = modpos.x() + 0.5 * width_38;
...
//   // VISUALIZATIONS
//   CDetLog->SetVisAttributes( G4VisAttributes::GetInvisible() );
//   CDetBarLog->SetVisAttributes( G4VisAttributes::GetInvisible() );
//   CDetModuleLog->SetVisAttributes( G4VisAttributes::GetInvisible() );
//   PlaneLog->SetVisAttributes( G4VisAttributes::GetInvisible() );
//   CDetModuleLog4->SetVisAttributes( G4VisAttributes::GetInvisible() );
...
//   //CDetMylarLog->SetVisAttributes( G4VisAttributes::GetInvisible() );
...
//   ecal_log->SetVisAttributes( G4VisAttributes::GetInvisible() );

//   //Mylar
//   G4VisAttributes *mylar_colour = new G4VisAttributes(G4Colour( 0.5, 0.5, 0.5 ) );
//   mylar_wrap_log->SetVisAttributes(G4VisAttributes::GetInvisible());

//   //Air
//   G4VisAttributes *air_colour = new G4VisAttributes(G4Colour( G4Colour::Blue() ));
//   module_log_type1->SetVisAttributes(G4VisAttributes::GetInvisible());
...
void G4SBSEArmBuilder::MakeBBSieveSlit(G4LogicalVolume *motherlog, G4ThreeVector pos)
{
  ...
  for (int irow = -6; irow <= 6; irow++)
  {
    ...
    for (int icol = -3; icol <= 3; icol++)
    {
      ...
      if (irow == -3 && (icol == -2 || icol == 1))
      { // Cut two rectangular slots between columns -2--1 and columns 1-2
...
void G4SBSEArmBuilder::MakeNewBBSieveSlit(G4LogicalVolume *motherlog, G4ThreeVector pos)
{
  ...
  for (G4int iy = -8; iy <= 8; iy++)
  {
    for (G4int ix = -5; ix <= 5; ix++)
    {
      ...
      {
        if ((iy == -3 && ix == -3) || (iy == 0 && ix == 0) || (iy == 2 && ix == 2))
...
void G4SBSEArmBuilder::MakeHallCGEM(G4LogicalVolume *motherlog)
{
  ...
  G4LogicalVolume *HCGEMlog = new G4LogicalVolume(HCGEMBox, GetMaterial("Air"),
                                                  "HCGEMLog", 0, 0, 0);
  HCGEMlog->SetVisAttributes(G4VisAttributes::GetInvisible());
...
void G4SBSEArmBuilder::MakeAxialFFNeutronArm(G4LogicalVolume *worldlog)
{
  ...
  G4ThreeVector postShift = (rotMatrix->inverse()).operator()(preShift);
  G4ThreeVector finalPosition = PosTarget + postShift;
...
void G4SBSEArmBuilder::MakeAxialFFNeutronArm_Magnet(G4LogicalVolume *motherlog)
{
  ...
  G4ThreeVector FieldVec(0, 0.5 * tesla, 0);
  ...
  G4UniformMagField *BField = new G4UniformMagField(FieldVec);
