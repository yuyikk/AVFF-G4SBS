                                                 ...
                                                 G4RotationMatrix *rot_targ, G4ThreeVector targ_offset)
{
  ...
  { // make a cylindrical target with half-spherical downstream endcap:
...
void G4SBSTargetBuilder::BuildOpticsTarget(G4LogicalVolume *motherlog, G4RotationMatrix *rot_targ, G4ThreeVector targ_offset)
{
  ...
  {
    if (fFoilZpos[ifoil] > zmax)
      zmax = fFoilZpos[ifoil];
    if (fFoilZpos[ifoil] < zmin)
      zmin = fFoilZpos[ifoil];
    if (fFoilThick[ifoil] > maxthick)
      maxthick = fFoilThick[ifoil];
  }
...
void G4SBSTargetBuilder::BuildStandardScatCham(G4LogicalVolume *worldlog)
{
  ...
  G4SubtractionSolid *solidSCFrontClam_0_rsbc =
      new G4SubtractionSolid("solidSCFrontClam_0_rsbc", solidSCFrontClam_0_rsb,
                             ...
                             G4ThreeVector((SCFrontClamOuterRadius + SCRightSnoutDepth) * sin(90.0 * deg - SCRightSnoutAngleOffset),
                                           (SCFrontClamOuterRadius + SCRightSnoutDepth) * cos(90.0 * deg - SCRightSnoutAngleOffset),
                                           0));
...
void G4SBSTargetBuilder::BuildGasTarget(G4LogicalVolume *worldlog)
{
  ...
  else if (fTargType == G4SBS::kCfoil)
  { // single carbon foil
    BuildCfoil(motherlog, 0, G4ThreeVector(0, 0, target_zpos));
  }
  else if (fTargType == G4SBS::kOptics)
  { // multi-foil (optics) target:
...
void G4SBSTargetBuilder::BuildGEnTarget(G4LogicalVolume *motherLog)
{
  ...
  if (config == G4SBS::kGEN_300)
    Q2 = 3.00;
  if (config == G4SBS::kGEN_683)
    Q2 = 6.83;
  if (config == G4SBS::kGEN_982)
    Q2 = 9.82;
  ...
  if (fDetCon->fExpType == G4SBS::kGEN || fDetCon->fExpType == G4SBS::kSIDISExp || fDetCon->fExpType == G4SBS::kALL)
  {
    ...
    if (enableIC)
      BuildGEnTarget_IonChamber(motherLog);
    ...
    if (enableBC_dnstr)
      BuildGEnTarget_BeamCollimator(motherLog, 0);
    if (enableBC_upstr)
      BuildGEnTarget_BeamCollimator(motherLog, 1);
...
void G4SBSTargetBuilder::BuildGEnTarget_GlassCell_old(G4LogicalVolume *motherLog)
{
  ...
  pumpCh.name = "pumpingChamber";
  pumpCh.shape = "sphere";
  pumpCh.r_tor = 0.0 * mm;
  pumpCh.r_max = 54.0 * mm;
  pumpCh.r_min = pumpCh.r_max - glassWall;
  pumpCh.length = 0.0 * mm;
  pumpCh.x_len = 0.0 * mm;
  pumpCh.y_len = 0.0 * mm;
  pumpCh.z_len = 0.0 * mm;
  pumpCh.startTheta = 0.0 * deg;
  pumpCh.dTheta = 180.0 * deg;
  pumpCh.startPhi = 0.0 * deg;
  pumpCh.dPhi = 360.0 * deg;
  pumpCh.x = 0.0 * mm;
  pumpCh.y = -330.2 * mm;
  pumpCh.z = 0.0 * mm;
  pumpCh.rx = 0.0 * deg;
  ...
  tgtCh.name = "targetChamber";
  tgtCh.shape = "tube";
  tgtCh.r_tor = 0.0 * mm;
  tgtCh.r_max = 10.5 * mm;
  tgtCh.r_min = tgtCh.r_max - glassWall;
  tgtCh.length = tubeLength;
  tgtCh.x_len = 0.0 * mm;
  tgtCh.y_len = 0.0 * mm;
  tgtCh.z_len = 0.0 * mm;
  tgtCh.startTheta = 0.0 * deg;
  tgtCh.dTheta = 0.0 * deg;
  tgtCh.startPhi = 0.0 * deg;
  tgtCh.dPhi = 360.0 * deg;
  tgtCh.x = 0.0 * mm;
  tgtCh.y = 0.0 * mm;
  tgtCh.z = 0.0 * mm;
  tgtCh.rx = 0.0 * deg;
  ...
  tted.name = "transTubeEl_dn";
  tted.shape = "torus";
  tted.r_tor = 9.0 * mm;
  tted.r_max = 4.5 * mm;
  tted.r_min = tted.r_max - glassWall;
  tted.length = 0.0 * mm;
  tted.x_len = 0.0 * mm;
  tted.y_len = 0.0 * mm;
  tted.z_len = 0.0 * mm;
  tted.startTheta = 0.0 * deg;
  tted.dTheta = 0.0 * deg;
  tted.startPhi = 0.0 * deg;
  tted.dPhi = 90.0 * deg;
  tted.x = 0.0 * mm;
  tted.y = -33.5 * mm;
  tted.z = 250.0 * mm;
  ...
  tteu.name = "transTubeEl_up";
  tteu.shape = "torus";
  tteu.r_tor = 9.0 * mm;
  tteu.r_max = 4.5 * mm;
  tteu.r_min = tteu.r_max - glassWall;
  tteu.length = 0.0 * mm;
  tteu.x_len = 0.0 * mm;
  tteu.y_len = 0.0 * mm;
  tteu.z_len = 0.0 * mm;
  tteu.startTheta = 0.0 * deg;
  tteu.dTheta = 0.0 * deg;
  tteu.startPhi = 0.0 * deg;
  tteu.dPhi = 90.0 * deg;
  tteu.x = 0.0 * mm;
  tteu.y = -33.5 * mm;
  tteu.z = -250.0 * mm;
  ...
  ttedl.name = "transTubeElLo_dn";
  ttedl.shape = "torus";
  ttedl.r_tor = 9.0 * mm;
  ttedl.r_max = 4.5 * mm;
  ttedl.r_min = ttedl.r_max - glassWall;
  ttedl.length = 0.0 * mm;
  ttedl.x_len = 0.0 * mm;
  ttedl.y_len = 0.0 * mm;
  ttedl.z_len = 0.0 * mm;
  ttedl.startTheta = 0.0 * deg;
  ttedl.dTheta = 0.0 * deg;
  ttedl.startPhi = 0.0 * deg;
  ttedl.dPhi = 90.0 * deg;
  ttedl.x = 0.0 * mm;
  ttedl.y = -51.5 * mm;
  ttedl.z = 34.5 * mm;
  ...
  tteul.name = "transTubeElLo_up";
  tteul.shape = "torus";
  tteul.r_tor = 9.0 * mm;
  tteul.r_max = 4.5 * mm;
  tteul.r_min = tteul.r_max - glassWall;
  tteul.length = 0.0 * mm;
  tteul.x_len = 0.0 * mm;
  tteul.y_len = 0.0 * mm;
  tteul.z_len = 0.0 * mm;
  tteul.startTheta = 0.0 * deg;
  tteul.dTheta = 0.0 * deg;
  tteul.startPhi = 0.0 * deg;
  tteul.dPhi = 90.0 * deg;
  tteul.x = 0.0 * mm;
  tteul.y = -51.5 * mm;
  tteul.z = -34.5 * mm;
  ...
  tts.name = "transTubeSphere";
  tts.shape = "sphere";
  tts.r_tor = 0.0 * mm;
  tts.r_max = 13.9 * mm;
  tts.r_min = tts.r_max - glassWall;
  tts.length = 0.0 * mm;
  tts.x_len = 0.0 * mm;
  tts.y_len = 0.0 * mm;
  tts.z_len = 0.0 * mm;
  tts.startTheta = 0.0 * deg;
  tts.dTheta = 180.0 * deg;
  tts.startPhi = 0.0 * deg;
  tts.dPhi = 360.0 * deg;
  tts.x = 0.0 * mm;
  tts.y = -83.3 * mm;
  tts.z = 25.4 * mm;
  ...
  ttuz.name = "transTubeZ_up";
  ttuz.shape = "tube";
  ttuz.r_tor = 0.0 * mm;
  ttuz.r_max = 4.5 * mm;
  ttuz.r_min = ttuz.r_max - glassWall;
  ttuz.length = 216.0 * mm;
  ttuz.x_len = 0.0 * mm;
  ttuz.y_len = 0.0 * mm;
  ttuz.z_len = 0.0 * mm;
  ttuz.startTheta = 0.0 * deg;
  ttuz.dTheta = 0.0 * deg;
  ttuz.startPhi = 0.0 * deg;
  ttuz.dPhi = 360.0 * deg;
  ttuz.x = 0.0 * mm;
  ttuz.y = -42.5 * mm;
  ttuz.z = -142.0 * mm;
  ...
  ttdz.name = "transTubeZ_dn";
  ttdz.shape = "tube";
  ttdz.r_tor = 0.0 * mm;
  ttdz.r_max = 4.5 * mm;
  ttdz.r_min = ttdz.r_max - glassWall;
  ttdz.length = 216.0 * mm;
  ttdz.x_len = 0.0 * mm;
  ttdz.y_len = 0.0 * mm;
  ttdz.z_len = 0.0 * mm;
  ttdz.startTheta = 0.0 * deg;
  ttdz.dTheta = 0.0 * deg;
  ttdz.startPhi = 0.0 * deg;
  ttdz.dPhi = 360.0 * deg;
  ttdz.x = 0.0 * mm;
  ttdz.y = -42.5 * mm;
  ttdz.z = 142.0 * mm;
  ...
  ttuy.name = "transTubeY_up";
  ttuy.shape = "tube";
  ttuy.r_tor = 0.0 * mm;
  ttuy.r_max = 4.5 * mm;
  ttuy.r_min = ttuy.r_max - glassWall;
  ttuy.length = 233.0 * mm;
  ttuy.x_len = 0.0 * mm;
  ttuy.y_len = 0.0 * mm;
  ttuy.z_len = 0.0 * mm;
  ttuy.startTheta = 0.0 * deg;
  ttuy.dTheta = 0.0 * deg;
  ttuy.startPhi = 0.0 * deg;
  ttuy.dPhi = 360.0 * deg;
  ttuy.x = 0.0 * mm;
  ttuy.y = -168.0 * mm;
  ttuy.z = -25.4 * mm;
  ...
  ttdby.name = "transTubeYB_dn";
  ttdby.shape = "tube";
  ttdby.r_tor = 0.0 * mm;
  ttdby.r_max = 4.5 * mm;
  ttdby.r_min = ttdby.r_max - glassWall;
  ttdby.length = 189.0 * mm;
  ttdby.x_len = 0.0 * mm;
  ttdby.y_len = 0.0 * mm;
  ttdby.z_len = 0.0 * mm;
  ttdby.startTheta = 0.0 * deg;
  ttdby.dTheta = 0.0 * deg;
  ttdby.startPhi = 0.0 * deg;
  ttdby.dPhi = 360.0 * deg;
  ttdby.x = 0.0 * mm;
  ttdby.y = -190.0 * mm;
  ttdby.z = 25.4 * mm;
  ttdby.rx = 90.0 * deg;
  ...
  ttday.name = "transTubeYA_dn";
  ttday.shape = "tube";
  ttday.r_tor = 0.0 * mm;
  ttday.r_max = 4.5 * mm;
  ttday.r_min = ttday.r_max - glassWall;
  ttday.length = 20.0 * mm;
  ttday.x_len = 0.0 * mm;
  ttday.y_len = 0.0 * mm;
  ttday.z_len = 0.0 * mm;
  ttday.startTheta = 0.0 * deg;
  ttday.dTheta = 0.0 * deg;
  ttday.startPhi = 0.0 * deg;
  ttday.dPhi = 360.0 * deg;
  ttday.x = 0.0 * mm;
  ttday.y = -61.0 * mm;
  ttday.z = 25.4 * mm;
  ttday.rx = 90.0 * deg;
  ...
  ttpdy.name = "transTubePost_dn";
  ttpdy.shape = "tube";
  ttpdy.r_tor = 0.0 * mm;
  ttpdy.r_max = 4.5 * mm;
  ttpdy.r_min = ttpdy.r_max - glassWall;
  ttpdy.length = 23.0 * mm;
  ttpdy.x_len = 0.0 * mm;
  ttpdy.y_len = 0.0 * mm;
  ttpdy.z_len = 0.0 * mm;
  ttpdy.startTheta = 0.0 * deg;
  ttpdy.dTheta = 0.0 * deg;
  ttpdy.startPhi = 0.0 * deg;
  ttpdy.dPhi = 360.0 * deg;
  ttpdy.x = 0.0 * mm;
  ttpdy.y = -22.0 * mm;
  ttpdy.z = 259.1 * mm;
  ttpdy.rx = 90.0 * deg;
  ...
  ttpuy.name = "transTubePost_up";
  ttpuy.shape = "tube";
  ttpuy.r_tor = 0.0 * mm;
  ttpuy.r_max = 4.5 * mm;
  ttpuy.r_min = ttpuy.r_max - glassWall;
  ttpuy.length = 23.0 * mm;
  ttpuy.x_len = 0.0 * mm;
  ttpuy.y_len = 0.0 * mm;
  ttpuy.z_len = 0.0 * mm;
  ttpuy.startTheta = 0.0 * deg;
  ttpuy.dTheta = 0.0 * deg;
  ttpuy.startPhi = 0.0 * deg;
  ttpuy.dPhi = 360.0 * deg;
  ttpuy.x = 0.0 * mm;
  ttpuy.y = -22.0 * mm;
  ttpuy.z = -259.1 * mm;
  ttpuy.rx = 90.0 * deg;
...
void G4SBSTargetBuilder::BuildGEnTarget_GlassCell(G4LogicalVolume *motherLog)
{
  ...
  pumpCh.shape = "sphere";
  pumpCh.r_tor = 0.0 * mm;
  pumpCh.r_max = pumpCh_OD / 2;
  pumpCh.r_min = pumpCh.r_max - pumpCh_glassWall;
  pumpCh.length = 0.0 * mm;
  pumpCh.x_len = 0.0 * mm;
  pumpCh.y_len = 0.0 * mm;
  pumpCh.z_len = 0.0 * mm;
  ...
  tgtCh.shape = "tube";
  tgtCh.r_tor = 0.0 * mm;
  tgtCh.r_max = tgtCh_OD / 2;
  tgtCh.r_min = tgtCh.r_max - tgtCh_glassWall;
  tgtCh.length = tubeLength;
  tgtCh.x_len = 0.0 * mm;
  tgtCh.y_len = 0.0 * mm;
  tgtCh.z_len = 0.0 * mm;
  ...
  ttul.name = "transTube_up_low";
  ttul.shape = "tube";
  ttul.r_tor = 0.0 * mm;
  ttul.r_max = tt_OD / 2;
  ttul.r_min = ttul.r_max - tt_glassWall;
  ttul.length = ttul_length + ttul_extra;
  ttul.x_len = 0.0 * mm;
  ttul.y_len = 0.0 * mm;
  ttul.z_len = 0.0 * mm;
  ttul.startTheta = 0.0 * deg;
  ttul.dTheta = 0.0 * deg;
  ttul.startPhi = 0.0 * deg;
  ttul.dPhi = 360.0 * deg;
  ttul.x = 0.0 * mm;
  ttul.y = -tgtCh_OD / 2 - (ttul_length + ttul_extra) / 2 + ttul_extra;
  ttul.z = -ttul_zpos;
  ttul.rx = 90.0 * deg;
  ttul.ry = 0.0 * deg;
  ttul.rz = 0.0 * deg;
  ...
  rm_ttul->rotateX(ttul.rx);
  rm_ttul->rotateY(ttul.ry);
  rm_ttul->rotateZ(ttul.rz);
  ...
  ttuh.name = "transTube_up_high";
  ttuh.shape = "tube";
  ttuh.r_tor = 0.0 * mm;
  ttuh.r_max = tt_OD / 2;
  ttuh.r_min = ttuh.r_max - tt_glassWall;
  ttuh.length = ttuh_length + ttuh_extra;
  ttuh.x_len = 0.0 * mm;
  ttuh.y_len = 0.0 * mm;
  ttuh.z_len = 0.0 * mm;
  ttuh.startTheta = 0.0 * deg;
  ttuh.dTheta = 0.0 * deg;
  ttuh.startPhi = 0.0 * deg;
  ttuh.dPhi = 360.0 * deg;
  ttuh.x = 0.0 * mm;
  ttuh.y = -pumpCh_ypos + pumpCh_OD / 2 - (ttuh_length + ttuh_extra) / 2 + ttuh_length;
  ttuh.z = -ttuh_zpos;
  ttuh.rx = 90.0 * deg;
  ttuh.ry = 0.0 * deg;
  ttuh.rz = 0.0 * deg;
  ...
  rm_ttuh->rotateX(ttuh.rx);
  rm_ttuh->rotateY(ttuh.ry);
  rm_ttuh->rotateZ(ttuh.rz);
  ...
  ttum.name = "transTube_up_mid";
  ttum.shape = "tube";
  ttum.r_tor = 0.0 * mm;
  ttum.r_max = tt_OD / 2;
  ttum.r_min = ttum.r_max - tt_glassWall;
  ttum.length = ttum_length;
  ttum.x_len = 0.0 * mm;
  ttum.y_len = 0.0 * mm;
  ttum.z_len = 0.0 * mm;
  ttum.startTheta = 0.0 * deg;
  ttum.dTheta = 0.0 * deg;
  ttum.startPhi = 0.0 * deg;
  ttum.dPhi = 360.0 * deg;
  ttum.x = 0.0 * mm;
  ttum.y = ttum_y;
  ttum.z = ttum_z;
  ...
  rm_ttum->rotateX(ttum.rx);
  rm_ttum->rotateY(ttum.ry);
  rm_ttum->rotateZ(ttum.rz);
  ...
  ttulel.name = "transTube_up_low_el";
  ttulel.shape = "torus";
  ttulel.r_tor = tt_OD / 2 * 1.01 * mm;
  ttulel.r_max = tt_OD / 2;
  ttulel.r_min = ttulel.r_max - tt_glassWall;
  ttulel.length = 0.0 * mm;
  ttulel.x_len = 0.0 * mm;
  ttulel.y_len = 0.0 * mm;
  ttulel.z_len = 0.0 * mm;
  ttulel.startTheta = 0.0 * deg;
  ttulel.dTheta = 0.0 * deg;
  ttulel.startPhi = 0.0 * deg;
  ttulel.dPhi = ttulel_ang;
  ttulel.x = 0.0 * mm;
  ttulel.y = ttulel_y;
  ttulel.z = ttulel_z;
  ttulel.rx = 180.0 * deg;
  ...
  rm_ttulel->rotateX(ttulel.rx);
  rm_ttulel->rotateY(ttulel.ry);
  rm_ttulel->rotateZ(ttulel.rz);
  ...
  ttuhel.name = "transTube_up_low_el";
  ttuhel.shape = "torus";
  ttuhel.r_tor = tt_OD / 2 * 1.01 * mm;
  ttuhel.r_max = tt_OD / 2;
  ttuhel.r_min = ttuhel.r_max - tt_glassWall;
  ttuhel.length = 0.0 * mm;
  ttuhel.x_len = 0.0 * mm;
  ttuhel.y_len = 0.0 * mm;
  ttuhel.z_len = 0.0 * mm;
  ttuhel.startTheta = 0.0 * deg;
  ttuhel.dTheta = 0.0 * deg;
  ttuhel.startPhi = 0.0 * deg;
  ttuhel.dPhi = ttuhel_ang;
  ttuhel.x = 0.0 * mm;
  ttuhel.y = ttuhel_y;
  ttuhel.z = ttuhel_z;
  ttuhel.rx = 0.0 * deg;
  ...
  rm_ttuhel->rotateX(ttuhel.rx);
  rm_ttuhel->rotateY(ttuhel.ry);
  rm_ttuhel->rotateZ(ttuhel.rz);
  ...
  rm_ttdl->rotateX(ttdl.rx);
  rm_ttdl->rotateY(ttdl.ry);
  rm_ttdl->rotateZ(ttdl.rz);
  ...
  rm_ttdh->rotateX(ttdh.rx);
  rm_ttdh->rotateY(ttdh.ry);
  rm_ttdh->rotateZ(ttdh.rz);
  ...
  rm_ttdm->rotateX(ttdm.rx);
  rm_ttdm->rotateY(ttdm.ry);
  rm_ttdm->rotateZ(ttdm.rz);

  // transfer tube downstream lower elbow
  partParameters_t ttdlel = ttulel;
  ttdlel.name = "transTube_dn_low_el";
  ttdlel.shape = "torus";
  ttdlel.z *= -1.;
  ttdlel.rx *= -1.;
  ttdlel.ry *= -1.;
  ...
  rm_ttdlel->rotateX(ttdlel.rx);
  rm_ttdlel->rotateY(ttdlel.ry);
  rm_ttdlel->rotateZ(ttdlel.rz);
  ...
  rm_ttdhel->rotateX(ttdhel.rx);
  rm_ttdhel->rotateY(ttdhel.ry);
  rm_ttdhel->rotateZ(ttdhel.rz);

  /// We now build the ends of the target chamber

  // Upstream cap first
  partParameters_t ttcapu;
  ttcapu.name = "targetChamber_cap_up";
  ttcapu.shape = "sphere";
  ttcapu.r_tor = 0.0 * mm;
  ttcapu.r_max = tgtCh_OD / 2;
  ttcapu.r_min = ttcapu.r_max - tgtCh_glassWall;
  ttcapu.length = 0.0 * mm;
  ttcapu.x_len = 0.0 * mm;
  ttcapu.y_len = 0.0 * mm;
  ttcapu.z_len = 0.0 * mm;
  ttcapu.startTheta = 0 * deg;
  ttcapu.dTheta = 90 * deg;
  ttcapu.startPhi = 0.0 * deg;
  ttcapu.dPhi = 360.0 * deg;
  ttcapu.x = 0.0 * mm;
  ttcapu.y = 0.0 * mm;
  ttcapu.z = -tgtCh.length / 2;
  ttcapu.rx = 180.0 * deg;
  ttcapu.ry = 0.0 * deg;
  ttcapu.rz = 0.0 * deg;
  ...
  rm_ttcapu->rotateX(ttcapu.rx);
  rm_ttcapu->rotateY(ttcapu.ry);
  rm_ttcapu->rotateZ(ttcapu.rz);
  ...
  rm_ttcapd->rotateX(ttcapd.rx);
  rm_ttcapd->rotateY(ttcapd.ry);
  rm_ttcapd->rotateZ(ttcapd.rz);
...
void G4SBSTargetBuilder::BuildGEnTarget_EndWindows_CuAl(G4LogicalVolume *motherLog)
{
  ...
  msh.name = "ew_mainShaft";
  msh.shape = "tube";
  msh.r_tor = 0.000 * inch;
  msh.r_min = 0.471 * inch;
  msh.r_max = 0.505 * inch;
  msh.length = 0.500 * inch;
  msh.x_len = 0.000 * inch;
  msh.y_len = 0.000 * inch;
  msh.z_len = 0.000 * inch;
  msh.startTheta = 0.000 * deg;
  msh.dTheta = 0.000 * deg;
  msh.startPhi = 0.000 * deg;
  msh.dPhi = 360.000 * deg;
  msh.x = 0.000 * inch;
  msh.y = 0.000 * inch;
  msh.z = 0.000 * inch;
  msh.rx = 0.000 * deg;
  ...
  msh_il.name = "ew_mainShaft_il";
  msh.shape = "tube";
  msh_il.r_tor = 0.000 * inch;
  msh_il.r_min = 0.4710 * inch;
  msh_il.r_max = 0.4725 * inch;
  msh_il.length = 0.125 * inch;
  msh_il.x_len = 0.000 * inch;
  msh_il.y_len = 0.000 * inch;
  msh_il.z_len = 0.000 * inch;
  msh_il.startTheta = 0.000 * deg;
  msh_il.dTheta = 0.000 * deg;
  msh_il.startPhi = 0.000 * deg;
  msh_il.dPhi = 360.000 * deg;
  msh_il.x = 0.000 * inch;
  msh_il.y = 0.000 * inch;
  msh_il.z = 0.5 * msh.length + 0.5 * msh_il.length;
  msh_il.rx = 0.000 * deg;
  msh_il.ry = 0.000 * deg;
  msh_il.rz = 0.000 * deg;
  ...
  rm_mshil->rotateX(msh_il.rx);
  rm_mshil->rotateY(msh_il.ry);
  rm_msh->rotateZ(msh_il.rz);
  ...
  lip.name = "ew_lip";
  lip.shape = "tube";
  lip.r_tor = 0.000 * inch;
  lip.r_min = 0.4725 * inch;
  lip.r_max = 0.6105 * inch;
  lip.length = 0.125 * inch;
  lip.x_len = 0.000 * inch;
  lip.y_len = 0.000 * inch;
  lip.z_len = 0.000 * inch;
  lip.startTheta = 0.000 * deg;
  lip.dTheta = 0.000 * deg;
  lip.startPhi = 0.000 * deg;
  lip.dPhi = 360.000 * deg;
  lip.x = 0.000 * inch;
  lip.y = 0.000 * inch;
  lip.z = 0.259 * inch;
  lip.rx = 0.000 * deg;
  ...
  rlip.name = "ew_rlip";
  rlip.shape = "sphere";
  rlip.r_tor = 0.000 * inch;
  rlip.r_min = 0.4855 * inch;
  rlip.r_max = 0.5455 * inch;
  rlip.length = 0.2468 * inch;
  rlip.x_len = 0.000 * inch;
  rlip.y_len = 0.000 * inch;
  rlip.z_len = 0.000 * inch;
  rlip.startTheta = 63.100 * deg;
  rlip.dTheta = 26.900 * deg;
  rlip.startPhi = 0.000 * deg;
  rlip.dPhi = 360.000 * deg;
  rlip.x = 0.000 * inch;
  rlip.y = 0.000 * inch;
  rlip.z = 0.5 * lip.length;
  rlip.rx = 0.000 * deg;
  rlip.ry = 0.000 * deg;
  rlip.rz = 0.000 * deg;
  ...
  ec.shape = "sphere";
  ec.r_tor = 0.000 * inch;
  ec.r_min = 0.4855 * inch;
  ec.r_max = 0.4915 * inch;
  ec.length = 0.2447 * inch;
  ec.x_len = 0.000 * inch;
  ec.y_len = 0.000 * inch;
  ec.z_len = 0.000 * inch;
  ec.startTheta = 0.000 * deg;
  ec.dTheta = 63.100 * deg;
  ec.startPhi = 0.000 * deg;
  ec.dPhi = 360.000 * deg;
  ec.x = 0.000 * inch;
  ec.y = 0.000 * inch;
  ec.z = 0.5 * lip.length;
  ...
  G4double COS_G = cos(drx);
  G4double COS_B = cos(dry);
  G4double COS_A = cos(drz);
  G4double SIN_G = sin(drx);
  G4double SIN_B = sin(dry);
  G4double SIN_A = sin(drz);
...
void G4SBSTargetBuilder::BuildGEnTarget_EndWindows_solidCu(G4LogicalVolume *motherLog)
{
  ...
  lip.shape = "tube";
  lip.r_tor = 0.000 * inch;
  lip.r_min = 0.471 * inch;
  lip.r_max = 0.611 * inch;
  lip.length = 0.125 * inch;
  lip.x_len = 0.000 * inch;
  lip.y_len = 0.000 * inch;
  lip.z_len = 0.000 * inch;
...
void G4SBSTargetBuilder::BuildGEnTarget_PolarizedHe3_old(G4LogicalVolume *motherLog)
{
  ...
  partParameters_t tc;
  tc.name = "targetChamber";
  tc.shape = "tube";
  tc.r_tor = 0. * mm;
  tc.r_min = 0. * mm;
  tc.r_max = 10.5 * mm - glassWall;
  tc.length = tubeLength;
  tc.startTheta = 0 * deg;
  tc.dTheta = 0 * deg;
  tc.startPhi = 0 * deg;
  tc.dPhi = 360 * deg;
  ...
  mshu.r_tor = 0.0 * mm;
  mshu.r_min = 0.0 * mm;
  mshu.r_max = 0.451 * inch;
  mshu.length = 0.500 * inch;
  mshu.startTheta = 0.0 * deg;
  mshu.dTheta = 0.0 * deg;
  mshu.startPhi = 0.0 * deg;
  mshu.dPhi = 360.0 * deg;
  // mshu.x = 0.0*mm; mshu.y = 0.0*mm; mshu.z = -11.504*inch;
  mshu.x = 0.0 * mm;
  mshu.y = 0.0 * mm;
  mshu.z = (-1.) * (tubeLength / 2. + 0.5 * mshu.length);
  mshu.rx = 0.0 * deg;
  mshu.ry = 0.0 * deg;
  mshu.rz = 0.0 * deg;
  ...
  lipu.r_tor = 0.0 * mm;
  lipu.r_min = 0.0 * mm;
  lipu.r_max = 0.451 * inch;
  lipu.length = 0.125 * inch;
  lipu.startTheta = 0.0 * deg;
  lipu.dTheta = 0.0 * deg;
  lipu.startPhi = 0.0 * deg;
  lipu.dPhi = 360.0 * deg;
  // lipu.x = 0.0*mm; lipu.y = 0.0*mm; lipu.z = -11.763*inch;
  lipu.x = 0.0 * mm;
  lipu.y = 0.0 * mm;
  lipu.z = (-1.) * (tubeLength / 2. + mshu.length + 0.5 * lipu.length);
  lipu.rx = 0.0 * deg;
  lipu.ry = 0.0 * deg;
  lipu.rz = 0.0 * deg;
  ...
  segu.name = "seg_up";
  segu.shape = "sphere";
  segu.r_tor = 0.000 * inch;
  segu.r_min = 0.000 * inch;
  segu.r_max = 0.4855 * inch - delta;
  segu.length = 0.000 * inch;
  segu.x_len = 0.000 * inch;
  segu.y_len = 0.000 * inch;
  segu.z_len = 0.000 * inch;
  segu.startTheta = 0.000 * deg;
  segu.dTheta = 90.000 * deg;
  segu.startPhi = 0.000 * deg;
  segu.dPhi = 360.000 * deg;
  // segu.x = 0.000*inch; segu.y = 0.000*inch; segu.z = -11.804*inch;
  segu.x = 0.000 * inch;
  segu.y = 0.000 * inch;
  segu.z = (-1.) * (tubeLength / 2. + mshu.length + lipu.length);
  segu.rx = 0.000 * deg;
  segu.ry = 180.000 * deg;
  segu.rz = 0.000 * deg;
...
void G4SBSTargetBuilder::BuildGEnTarget_PolarizedHe3(G4LogicalVolume *motherLog)
{
  ...
  tc.r_tor = 0. * mm;
  tc.r_min = 0. * mm;
  tc.r_max = tgtCh_OD / 2 - tgtCh_glassWall;
  tc.length = tubeLength;
  tc.startTheta = 0 * deg;
  tc.dTheta = 0 * deg;
  tc.startPhi = 0 * deg;
  ...
  // Upstream cap first
  partParameters_t ttcapu;
  ttcapu.name = "He3_cap_up";
  ttcapu.shape = "sphere";
  ttcapu.r_tor = 0.0 * mm;
  ttcapu.r_max = tc.r_max;
  ttcapu.r_min = 0;
  ttcapu.length = 0.0 * mm;
  ttcapu.x_len = 0.0 * mm;
  ttcapu.y_len = 0.0 * mm;
...
void G4SBSTargetBuilder::BuildGEnTarget_HelmholtzCoils(const int config, const std::string type, G4LogicalVolume *motherLog)
{
  ...
  if (type.compare("maj") == 0)
    shWall = 5.0 * mm; // FIXME: Estimates for now!
  if (type.compare("min") == 0)
    shWall = 5.0 * mm; // FIXME: Estimates for now!
  if (type.compare("rfy") == 0)
    shWall = 0.030 * inch;
  ...
  if (type.compare("maj") == 0 || type.compare("min") == 0)
  {
    if (config == G4SBS::kGEN_300)
      dry = 46.2 * deg;
    if (config == G4SBS::kGEN_683 || config == G4SBS::kGEN_982)
      dry = 7.8 * deg;
  }
  ...
  if (type.compare("maj") == 0)
    visCS->SetColour(G4Colour::Red());
  if (type.compare("rfy") == 0)
    visCS->SetColour(G4Colour::Green());
  if (type.compare("min") == 0)
    visCS->SetColour(G4Colour::Blue());
...
void G4SBSTargetBuilder::BuildGEnTarget_Shield(const int config, G4LogicalVolume *motherLog)
{
  ...
  for (int i = 0; i < 2; i++)
  {
    ...
    if (i == 0)
      sprintf(physName, "physGEnTarget_InnerShield");
    if (i == 1)
      sprintf(physName, "physGEnTarget_OuterShield");
...
void G4SBSTargetBuilder::BuildGEnTarget_PickupCoils(G4LogicalVolume *motherLog)
{
  ...
  cbul.y_len = 48.0 * mm;
  cbul.z_len = 67.0 * mm;
  cbul.startTheta = 0.0 * deg;
  cbul.dTheta = 0.0 * deg;
  cbul.startPhi = 0.0 * deg;
  cbul.dPhi = 0.0 * deg;
  cbul.x = 0.0 * mm;
  cbul.y = -24.0 * mm;
  cbul.z = 0.0 * mm;
  ...
  cmul.y_len = 30.0 * mm;
  cmul.z_len = 120.0 * mm;
  cmul.startTheta = 0.0 * deg;
  cmul.dTheta = 0.0 * deg;
  cmul.startPhi = 0.0 * deg;
  cmul.dPhi = 0.0 * deg;
  cmul.x = 0.0 * mm;
  cmul.y = 15.0 * mm;
  cmul.z = 0.0 * mm;
...
void G4SBSTargetBuilder::BuildGEnTarget_LadderPlate(G4LogicalVolume *motherLog)
{
  ...
  lvu.name = "ladder_vert_up";
  lvu.shape = "box";
  lvu.r_tor = 0.0 * mm;
  lvu.r_min = 0.0 * mm;
  lvu.r_max = 0.0 * mm;
  lvu.length = 0.0 * mm;
  lvu.x_len = plate_width;
  lvu.y_len = 203.2 * mm;
  lvu.z_len = 38.1 * mm;
  lvu.startTheta = 0.0 * deg;
  lvu.dTheta = 0.0 * deg;
  lvu.startPhi = 0.0 * deg;
  lvu.dPhi = 0.0 * deg;
  lvu.x = -11.1 * mm;
  lvu.y = 0.0 * mm;
  lvu.z = -363.0 * mm;
  ...
  rm_lvu->rotateX(lvu.rx);
  rm_lvu->rotateY(lvu.ry);
  rm_lvu->rotateZ(lvu.rz);

  // ---- downstream
  partParameters_t lvd;
  lvd.name = "ladder_vert_dn";
  lvd.shape = "box";
  lvd.r_tor = 0.0 * mm;
  lvd.r_min = 0.0 * mm;
  lvd.r_max = 0.0 * mm;
  lvd.length = 0.0 * mm;
  lvd.x_len = plate_width;
  lvd.y_len = 203.2 * mm;
  lvd.z_len = 38.1 * mm;
  lvd.startTheta = 0.0 * deg;
  lvd.dTheta = 0.0 * deg;
  lvd.startPhi = 0.0 * deg;
  lvd.dPhi = 0.0 * deg;
  lvd.x = 0.0 * mm;
  lvd.y = 0.0 * mm;
  lvd.z = 363.0 * mm;
  ...
  rm_lvd->rotateX(lvd.rx);
  rm_lvd->rotateY(lvd.ry);
  rm_lvd->rotateZ(lvd.rz);

  // horizontal posts along the z axis
  // ---- above
  partParameters_t la;
  la.name = "ladder_above";
  la.shape = "box";
  la.r_tor = 0.0 * mm;
  la.r_min = 0.0 * mm;
  la.r_max = 0.0 * mm;
  la.length = 0.0 * mm;
  la.x_len = plate_width;
  la.y_len = 38.1 * mm;
  la.z_len = 370.0 * mm;
  la.startTheta = 0.0 * deg;
  la.dTheta = 0.0 * deg;
  la.startPhi = 0.0 * deg;
  la.dPhi = 0.0 * deg;
  la.x = 0.0 * mm;
  ...
  // ---- below
  partParameters_t lb;
  lb.name = "ladder_below";
  lb.shape = "box";
  lb.r_tor = 0.0 * mm;
  lb.r_min = 0.0 * mm;
  lb.r_max = 0.0 * mm;
  lb.length = 0.0 * mm;
  lb.x_len = plate_width;
  lb.y_len = 38.1 * mm;
  lb.z_len = 114.3 * mm;
  lb.startTheta = 0.0 * deg;
  lb.dTheta = 0.0 * deg;
  lb.startPhi = 0.0 * deg;
  lb.dPhi = 0.0 * deg;
  lb.x = 0.0 * mm;
  ...
  rm_lb->rotateX(lb.rx);
  rm_lvd->rotateY(lb.ry);
  rm_lb->rotateZ(lb.rz);

  // angular part above, along z axis
  // ---- upstream
  partParameters_t aau;
  aau.name = "ladder_ang_above_up";
  aau.shape = "box";
  aau.r_tor = 0.0 * mm;
  aau.r_min = 0.0 * mm;
  aau.r_max = 0.0 * mm;
  aau.length = 0.0 * mm;
  aau.x_len = plate_width;
  aau.y_len = 38.1 * mm;
  aau.z_len = 240.0 * mm;
  aau.startTheta = 0.0 * deg;
  aau.dTheta = 0.0 * deg;
  aau.startPhi = 0.0 * deg;
  aau.dPhi = 0.0 * deg;
  aau.x = 0.0 * mm;
  aau.y = 150.0 * mm;
  aau.z = 70.4 * mm;
  ...
  rm_aau->rotateX(aau.rx);
  rm_aau->rotateY(aau.ry);
  rm_aau->rotateZ(aau.rz);

  // ---- downstream
  partParameters_t aad;
  aad.name = "ladder_ang_above_dn";
  aad.shape = "box";
  aad.r_tor = 0.0 * mm;
  aad.r_min = 0.0 * mm;
  aad.r_max = 0.0 * mm;
  aad.length = 0.0 * mm;
  aad.x_len = plate_width;
  aad.y_len = 38.1 * mm;
  aad.z_len = 280.0 * mm;
  aad.startTheta = 0.0 * deg;
  aad.dTheta = 0.0 * deg;
  aad.startPhi = 0.0 * deg;
  aad.dPhi = 0.0 * deg;
  aad.x = 0.0 * mm;
  aad.y = 150.0 * mm;
  aad.z = 600.0 * mm;
  ...
  rm_aad->rotateX(aad.rx);
  rm_aad->rotateY(aad.ry);
  rm_aad->rotateZ(aad.rz);

  // angular part below, along z axis
  // ---- upstream
  partParameters_t abu;
  abu.name = "ladder_ang_below_up";
  abu.shape = "box";
  abu.r_tor = 0.0 * mm;
  abu.r_min = 0.0 * mm;
  abu.r_max = 0.0 * mm;
  abu.length = 0.0 * mm;
  abu.x_len = plate_width;
  abu.y_len = 38.1 * mm;
  abu.z_len = 282.0 * mm;
  abu.startTheta = 0.0 * deg;
  abu.dTheta = 0.0 * deg;
  abu.startPhi = 0.0 * deg;
  abu.dPhi = 0.0 * deg;
  abu.x = 0.0 * mm;
  abu.y = -125.0 * mm;
  abu.z = 131.0 * mm;
  ...
  rm_abu->rotateX(abu.rx);
  rm_abu->rotateY(abu.ry);
  rm_abu->rotateZ(abu.rz);

  // ---- downstream
  partParameters_t abd;
  abd.name = "ladder_ang_below_dn";
  abd.shape = "box";
  abd.r_tor = 0.0 * mm;
  abd.r_min = 0.0 * mm;
  abd.r_max = 0.0 * mm;
  abd.length = 0.0 * mm;
  abd.x_len = plate_width;
  abd.y_len = 38.1 * mm;
  abd.z_len = 350.0 * mm;
  abd.startTheta = 0.0 * deg;
  abd.dTheta = 0.0 * deg;
  abd.startPhi = 0.0 * deg;
  abd.dPhi = 0.0 * deg;
  abd.x = 0.0 * mm;
  abd.y = -125.0 * mm;
  abd.z = 540.0 * mm;
  ...
  rm_abd->rotateX(abd.rx);
  rm_abd->rotateY(abd.ry);
  rm_abd->rotateZ(abd.rz);
...
void G4SBSTargetBuilder::BuildGEnTarget_Collimator_B(G4LogicalVolume *logicMother, G4double z0)
{
  ...
  POS.push_back(2.520 * inch);
  POS.push_back(-0.55 * inch);
  POS.push_back(z0 + 14.2 * inch);
  G4ThreeVector P = G4ThreeVector(POS[0], POS[1], POS[2]);
  // rotation
  std::vector<G4double> RA;
  RA.push_back(0. * deg);
  RA.push_back(-47.24 * deg);
  RA.push_back(0. * deg);
  G4RotationMatrix *rm = new G4RotationMatrix();
  rm->rotateX(RA[0]);
  rm->rotateY(RA[1]);
...
void G4SBSTargetBuilder::BuildGEnTarget_Collimator_C(G4LogicalVolume *logicMother, G4double z0)
{
  ...
  // position
  std::vector<G4double> POS;
  POS.push_back(4.070 * inch);
  POS.push_back(-0.35 * inch);
  POS.push_back(z0 + 20.870 * inch + 0.4 * inch);
  G4ThreeVector P = G4ThreeVector(POS[0], POS[1], POS[2]);
  // rotation
  std::vector<G4double> RA;
  RA.push_back(0. * deg);
  RA.push_back(-50.41 * deg);
  RA.push_back(0. * deg);
  G4RotationMatrix *rm = new G4RotationMatrix();
  rm->rotateX(RA[0]);
  rm->rotateY(RA[1]);
...
void G4SBSTargetBuilder::BuildAVFFTarget(G4LogicalVolume *motherlog)
{
  ...
  const G4double LChamber = LenContainer + 10 * cm;

  G4Tubs *solidChamber = new G4Tubs("solidChamber", 0, RChamber, LChamber / 2., 0, CLHEP::twopi);
  G4LogicalVolume *logicChamber = new G4LogicalVolume(solidChamber, GetMaterial("Vacuum"), "logicChamber");
  new G4PVPlacement(0, PosTarget, logicChamber, "physChamber", motherlog, false, 0, true);
  G4Tubs *tub = new G4Tubs("tub", 0, RContainer, LenContainer / 2., 0, CLHEP::twopi);
  G4Tubs *solidTarget = new G4Tubs("solidTarget", 0, RTarget, LenTarget / 2., 0, CLHEP::twopi);
  G4SubtractionSolid *solidContainer = new G4SubtractionSolid("solidContainer", tub, solidTarget);
  G4LogicalVolume *logicContainer = new G4LogicalVolume(solidContainer, GetMaterial("Aluminum"), "logicContainer");
  G4LogicalVolume *logicTarget = new G4LogicalVolume(solidTarget, GetMaterial("LH2"), "logicTarget");

  new G4PVPlacement(0, PosTarget, logicContainer, "physContainer", logicChamber, false, 0, true);
  new G4PVPlacement(0, PosTarget, logicTarget, "physTarget", logicChamber, false, 0, true);

  fDetCon->InsertTargetVolume(logicContainer->GetName());
  fDetCon->InsertTargetVolume(logicTarget->GetName());
  G4VisAttributes *TargetVisAtt = new G4VisAttributes(G4Color::Cyan());
  TargetVisAtt->SetForceSolid(true);
  logicTarget->SetVisAttributes(TargetVisAtt);
}

void G4SBSTargetBuilder::BuildAVFFCollimator(G4LogicalVolume *motherLog)
{

  auto Tungsten = G4Material::GetMaterial("G4_W");
  G4double coneHeight = MyCollimator::kThickness / std::tan(MyCollimator::kConeAngle / 2.);
  G4double totallength = MyCollimator::kLength + coneHeight;
  G4Tubs *solidCollimator = new G4Tubs("solidCollimator", MyCollimator::kInnerR, MyCollimator::kOuterR, totallength / 2., -90 * deg, CLHEP::pi);

  G4Cons *solidCone = new G4Cons("solidCone", 0, MyCollimator::kOuterR, 0, MyCollimator::kInnerR, coneHeight / 2., -90 * deg, CLHEP::pi);

  G4Tubs *solid_tmp = new G4Tubs("solid_tmp", 0, MyCollimator::kInnerR,
                                 coneHeight / 2, 0, CLHEP::twopi);

  G4SubtractionSolid *solidCone_Sub = new G4SubtractionSolid("solidCone_Sub", solidCone, solid_tmp);

  G4ThreeVector conPos(0, 0, coneHeight / 2. - totallength / 2.);
  G4SubtractionSolid *solidCollimator_Sub = new G4SubtractionSolid("solidCollimator_Sub", solidCollimator, solidCone_Sub, nullptr, conPos);
