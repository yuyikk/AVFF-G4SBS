...
*/
  ...
  if (fDetCon->fBLneutronDet)
  { // TO-DO: set the possibility to deactivate it.
...
void G4SBSBeamlineBuilder::Make3HeBeamline(G4LogicalVolume *worldlog)
{ // for GEn, A1n, SIDIS
...
void G4SBSBeamlineBuilder::MakeDefaultBeamline(G4LogicalVolume *worldlog)
{ // Old beam line...
...
void G4SBSBeamlineBuilder::MakeGEpLead(G4LogicalVolume *worldlog)
{
  ...
  G4bool fUseOldLeadWall1 = false;

  if (fUseOldLeadWall1 == true)
  {
    ...
    G4ThreeVector lead_wall2_pos(xpos_lead_wall2, 0, zpos_lead_wall2);
    G4ThreeVector lead_wall2_pos_gep2(xpos_lead_wall2 - sin(5.2 * deg) * zpos_lead_wall2, 0, zpos_lead_wall2 - (zpos_lead_wall2 - cos(5.2 * deg) * zpos_lead_wall2));
    G4ThreeVector lead_wall2_pos_gep1(xpos_lead_wall2 - sin(8.8 * deg) * zpos_lead_wall2, 0, zpos_lead_wall2 - (zpos_lead_wall2 - cos(8.8 * deg) * zpos_lead_wall2));

    new G4PVPlacement(rot_temp, lead_wall2_pos, lead_wall2_log, "lead_wall2_phys", worldlog, false, 0);
    ...
    G4ThreeVector lead_wall3_pos(xpos_lead_wall2 - (4.5 * inch), 0, zpos_lead_wall2 + (30.0 * inch) + (18.0 * inch));
    G4ThreeVector lead_wall3_pos_gep2(xpos_lead_wall2 - (4.5 * inch) - sin(5.2 * deg) * zpos_lead_wall2, 0, zpos_lead_wall2 + (30.0 * inch) + (18.0 * inch) - (zpos_lead_wall2 - cos(5.2 * deg) * zpos_lead_wall2));
    G4ThreeVector lead_wall3_pos_gep1(xpos_lead_wall2 - (6.5 * inch) - sin(8.8 * deg) * zpos_lead_wall2, 0, zpos_lead_wall2 + (30.0 * inch) + (18.0 * inch) - (zpos_lead_wall2 + (30 * inch) + (18.0 * inch) - cos(8.8 * deg) * (zpos_lead_wall2 + (30.0 * inch) + (18.0 * inch))));

    new G4PVPlacement(rot_temp, lead_wall3_pos, lead_wall3_log, "lead_wall3_phys", worldlog, false, 0);
...
void G4SBSBeamlineBuilder::MakeGMnLead(G4LogicalVolume *worldlog)
{
  ...
  if (leadring)
    new G4PVPlacement(rot_temp, G4ThreeVector(0, 0, z1_ringshield + th_ringshield / 2.0), ringshield_log, "ringshield_phys", worldlog, false, 0, checkoverlaps);
  ringshield_log->SetVisAttributes(LeadColor);
...
void G4SBSBeamlineBuilder::MakeBeamDump(G4LogicalVolume *logicMother, G4double dz)
{
  ...
  if (fDetCon->GetBeamDiffuserEnable())
...
void G4SBSBeamlineBuilder::MakeBeamDump_UpstreamPipe(G4LogicalVolume *logicMother, G4double z0)
{
  ...
  // - attach small cone
  zz = 0.5 * len_lgf + len_lgc + len_m2b + len_vtsr + len_m2 + len_apf + len_for + len_m6b + len_vtsr + len_m6 + 0.5 * len_smc;
  P_0 = G4ThreeVector(0, 0, -zz);
  ...
  zz = 0.5 * len_lgf + len_lgc + len_m2b + len_vtsr + len_m2 + len_apf + len_for + len_m6b + len_vtsr + len_m6 + len_smc + 0.5 * len_vwt;
  P_0 = G4ThreeVector(0, 0, -zz);
  ...
  zz = 0.5 * len_lgf + len_lgc + len_m2b + len_vtsr + len_m2 + len_apf + len_for + len_m6b + len_vtsr + len_m6 + len_smc + len_vwt + 0.5 * len_vsdf;
  P_0 = G4ThreeVector(0, 0, -zz);
  upstrPipe = new G4UnionSolid("upstreamPipe", upstrPipe, solidVSDF, 0, P_0);

  // this is a check
  G4double TOT_LEN_SUM = len_lgf + len_lgc + len_m2b + len_vtsr + len_m2 + len_apf + len_for + len_m6b + len_vtsr + len_m6 + len_smc + len_vwt + len_vsdf;
...
void G4SBSBeamlineBuilder::MakeBeamExit(G4LogicalVolume *logicMother, G4double dz)
{
  ...
  if (fDetCon->GetBeamDumpEnable())
    MakeBeamDump(logicMother, dz2);
}
...
void G4SBSBeamlineBuilder::MakeAVFFBeamline(G4LogicalVolume *motherlog)
{

  G4double Radius = 2.54 * cm + 1 * mm;
  const G4double RChamber = Radius;
  const G4double LChamber = 30 * cm + 0.3 * mm;

  G4Tubs *solidChamber = new G4Tubs("solidChamber", 0, RChamber, LChamber / 2., 0, CLHEP::twopi);
  G4Tubs *solidBeamline = new G4Tubs("solidBeamline", 0, Radius, 25 * m, 0, CLHEP::twopi);
  G4SubtractionSolid *solidBeamline_cut = new G4SubtractionSolid("solidBeamline_cut", solidBeamline, solidChamber);
  G4LogicalVolume *logicBeamline = new G4LogicalVolume(solidBeamline_cut, GetMaterial("Vacuum"), "logicBeamline");
  new G4PVPlacement(0, G4ThreeVector(0, 0, 0), logicBeamline, "physBeamline", motherlog, false, 0, true);
  ...
  logicBeamline->SetVisAttributes(BeamPipeVisAtt);
}
