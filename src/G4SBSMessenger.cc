...
G4SBSMessenger::G4SBSMessenger()
{
  ...
  expCmd->SetGuidance("Experiment type from gep, gmn, gen, a1n, sidis, C16, tdis, ndvcs, genrp, avff");
...
void G4SBSMessenger::SetNewValue(G4UIcommand *cmd, G4String newValue)
{
  ...
  if (cmd == runCmd)
  {
    ...
    if (fevgen->GetKine() == G4SBS::kAVFFGen)
    {
      long firstevt = fevgen->GetFirstEvent();
      long lastevt_chain = fevgen->GetAVFFGenChain()->GetEntries() - 1;
      nevt = std::min(long(nevt), lastevt_chain - firstevt + 1);
    ...
    switch (fdetcon->fTargType)
    { // Initialize fTargDen correctly and consistently with Material definition in fdetcon->ConstructMaterials:
    case G4SBS::kH2:
    ...
    if (kinetype == G4SBS::kDIS || kinetype == G4SBS::kWiser)
  ...
  if (cmd == kineCmd)
  {
    ...
    if (newValue.compareTo("avffgen") == 0)
    {
      kinetemp = G4SBS::kAVFFGen;
    ...
    if (newValue.compareTo("wapp") == 0)
    { // wide angle pion photoproduction
  ...
  {
    bool validcmd = false;
    if (newValue.compareTo("avff") == 0)
    {
      fExpType = G4SBS::kAxialFF;
      validcmd = true;
    }
  ...
  if (cmd == RejectionSamplingCmd)
  { // Turn on rejection sampling for built-in event generators (has no effect for Pythia/beam/etc)
  ...
  if (cmd == GRINCH_gas_Cmd)
  {
    ...
    { // default to C4F10 if no valid name given:
      gasname = "C4F10_gas";
  ...
  */
  ...
  if (cmd == KeepPartCALcmd)
  {
    ...
    if (newValue.contains("true") || newValue.contains("false"))
    { // parse with the "boolalpha" flag:
      is >> SDname >> std::boolalpha >> flag;
    }
    else
    { // assume that the boolean parameter is given as 1 or 0:
      is >> SDname >> flag;
    }
  ...
  // ******
  if (cmd == SD_EnergyThresholdCmd)
  { // store the SDname and dimensioned threshold value in a map<G4String,G4double> assigned to fdetcon?
    std::istringstream is(newValue);
  ...
  if (cmd == TargThetaSpinCmd)
  { // there must be at least nspin entries or this will fail:
    std::istringstream is(newValue);
