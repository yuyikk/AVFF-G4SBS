...
void G4SBSEventGen::LoadAVFFGenChain(G4String fname)
{
  if (fAVFFGenChain != NULL)
  {
...
void G4SBSEventGen::Initialize()
{
  ...
  if (fKineType == G4SBS::kWiser)
  { // Wiser generates flat in "hadron" solid angle and energy; i.e., it uses
  ...
  { // SIDIS generates flat in both electron and hadron solid angle and energy:
    fGenVol *= (fPhMax_had - fPhMin_had) * (cos(fThMin_had) - cos(fThMax_had)) * (fEhadMax - fEhadMin);
    fMaxWeight /= GeV;
    G4cout << "Generation volume = " << fGenVol / pow(GeV, 2) << "sr*GeV^2" << G4endl;
  }

  if (fKineType == G4SBS::kPionPhoto)
  { // Pion photoproduction generates flat in -t, phi, and Egamma and returns dsig/dt * Delta t * photon flux
...
bool G4SBSEventGen::GenerateEvent()
{
  ...
  { // vertex generation for multi-foil optics target:
  ...
  if (fRandomizeTargetSpin)
  {
    ...
    if (fNumTargetSpinDirections < 0)
    { // random in 3D:
      thspin = acos(CLHEP::RandFlat::shoot(-1.0, 1.0));
      phspin = CLHEP::RandFlat::shoot(-CLHEP::pi, CLHEP::pi);
    }
    else if (fNumTargetSpinDirections == 0)
    {                           // random in the plane perp. to beamline
      thspin = CLHEP::pi / 2.0; // polar angle 90 deg.
      phspin = CLHEP::RandFlat::shoot(-CLHEP::pi, CLHEP::pi);
    }
    else
    { // choose randomly among each of N discrete settings:
...
bool G4SBSEventGen::GenerateInelastic(G4SBS::Nucl_t nucl, G4LorentzVector ei, G4LorentzVector ni)
{
  ...
  */
  ...
  if (nucl == G4SBS::kNeutron && fFinalNucl == G4SBS::kNeutron)
  { // gamma n --> pi0 n
  ...
  else if (nucl == G4SBS::kProton && fFinalNucl == G4SBS::kProton)
  { // gamma p --> pi0 p
    Mpi = G4PionZero::PionZeroDefinition()->GetPDGMass();
    M_ni = proton_mass_c2;
    M_nf = M_ni;
  ...
  else if (nucl == G4SBS::kProton && fFinalNucl == G4SBS::kNeutron)
  { // gamma p --> pi+ n
  ...
  else if (nucl == G4SBS::kNeutron && fFinalNucl == G4SBS::kProton)
  { // gamma n --> pi- p
...
bool G4SBSEventGen::GenerateDIS(G4SBS::Nucl_t nucl, G4LorentzVector ei, G4LorentzVector ni)
{
  ...
  if (Pfsum_lab.m2() > Pisum_lab.m2() || Pfsum_lab.e() > Pisum_lab.e() ||
      x > 1.0 || x < 0.0)
  { // Check energy and momentum conservation for the generated kinematics:
    fSigma = 0.0;
    fHadronE = 0.0;
    fHadronP = G4ThreeVector();
    fElectronE = 0.0;
...
bool G4SBSEventGen::GenerateSIDIS(G4SBS::Nucl_t nucl, G4LorentzVector ei, G4LorentzVector ni)
{
  ...
  double H2 = x * b / CLHEP::twopi * exp(-b * pow(Ph_perp, 2)) * (pow(e_u, 2) * (u * Dqh[0] + ubar * Dqh[1]) + pow(e_d, 2) * (d * Dqh[2] + dbar * Dqh[3]) + pow(e_s, 2) * (st * Dqh[4] + sbar * Dqh[5]));

  if (nucl == G4SBS::kNeutron)
  { // Interchange u and d quarks: the d quark density in a neutron = u quark density in a proton etc.
    H2 = x * b / twopi * exp(-b * pow(Ph_perp, 2)) * (pow(e_u, 2) * (d * Dqh[0] + dbar * Dqh[1]) + pow(e_d, 2) * (u * Dqh[2] + ubar * Dqh[3]) + pow(e_s, 2) * (st * Dqh[4] + sbar * Dqh[5]));
  }
  ...
  if (phi_Collins < -CLHEP::pi)
    phi_Collins += CLHEP::twopi;
  if (phi_Collins > CLHEP::pi)
    phi_Collins -= CLHEP::twopi;
  if (phi_Sivers < -CLHEP::pi)
    phi_Sivers += CLHEP::twopi;
  if (phi_Sivers > CLHEP::pi)
    phi_Sivers -= CLHEP::twopi;
...
bool G4SBSEventGen::GenerateWiser(G4SBS::Nucl_t nucl, G4LorentzVector ei, G4LorentzVector ni)
{
  ...
  if (fSigma != 0.0)
  { // only generate kinematically allowed events to save CPU cycles
    return true;
  }
  else
  {
...
double G4SBSEventGen::he3pdist(G4SBS::Nucl_t nucl, double p)
{
  ...
  if (nucl == G4SBS::kProton)
  {
    ...
    {
      return ap2 * exp(-thisp * bp2) * (1.0 + cp1 * (thisp - pp0) + cp2 * pow(thisp - pp0, 2.0) + cp3 * pow(thisp - pp0, 3.0) + cp4 * pow(thisp - pp0, 4.0) + cp5 * pow(thisp - pp0, 5.0)) / pnorm;
    }
    else
    {
      return 0.0;
  ...
  if (nucl == G4SBS::kNeutron)
  {
    ...
    {
      return an2 * exp(-thisp * bn2) * (1.0 + cn1 * (thisp - pn0) + cn2 * pow(thisp - pn0, 2.0) + cn3 * pow(thisp - pn0, 3.0) + cn4 * pow(thisp - pn0, 4.0) + cn5 * pow(thisp - pn0, 5.0)) / nnorm;
    }
    else
    {
      return 0.0;
...
bool G4SBSEventGen::GeneratePythia()
{
  ...
  if (fchainentry % 1000 == 0)
    G4cout << "Passed event " << fchainentry << " in PYTHIA6 tree" << G4endl;
...
bool G4SBSEventGen::GenerateAVFFGen()
{
  Long64_t totalEvents = fAVFFEventGenTree->GetEntries() / 20;
  Long64_t step = 1;
  totalEvents > 10000 ? step = 10000 : step = totalEvents;
  if (fchainentry % step == 0)
  {
    G4cout << "Passed event " << fchainentry << " in AVFF generator tree" << G4endl;
  }
  fAVFFEventGenTree->GetEntry(fchainentry++);
  fAVFFGenEvent.Clear();
  fAVFFGenEvent.fWeight = fAVFFEventGenTree->GetWeight();

  TVector3 vertex = GenerateUniformVertex();
  TVector3 targetCenter(0, 0, 0);
  vertex += targetCenter;
  fAVFFGenEvent.fVx = vertex.x();
  fAVFFGenEvent.fVy = vertex.y();
  fAVFFGenEvent.fVz = vertex.z();
  const int nPart = fAVFFEventGenTree->GetNumOfParticle();
...
ev_t G4SBSEventGen::GetEventData()
{
  ...
  if (fKineType == G4SBS::kSIDIS)
  { // The SIDIS cross section is also differential in e- energy and hadron energy and has units of area/energy^2/sr^2, so we also need to express it in the correct energy units:
  ...
  if (fKineType == G4SBS::kSIDIS)
  { // Then replace final nucleon variables with final hadron variables:
...
void G4SBSEventGen::InitializeRejectionSampling()
{
  ...
  if (fRejectionSamplingFlag)
  {
    ...
    {
      if (i % 1000 == 0)
        G4cout << "Estimating max. event weight within generation limits, pre-event = " << i
...
void G4SBSEventGen::InitializePythia6_Tree()
{
  ...
  while ((chEl = (TChainElement *)next()))
  {
    ...
    if (gtemp)
    {
      ...
      while (!goodsigma && ipoint > 0)
      { // work backwards from the end of the array, take first cross section that isn't a "NAN"
        ipoint--;
...
void G4SBSEventGen::InitializeAVFFGen_Tree()
{
  fAVFFEventGenTree = new AVFFGenTree(fAVFFGenChain);
...
bool G4SBSEventGen::GeneratePionPhotoproduction(G4SBS::Nucl_t nucl, G4LorentzVector ei, G4LorentzVector ni)
{
  ...
  if (nucl == G4SBS::kNeutron)
  { // gamma n --> pi- p or gamma n --> pi0 n
    if (fHadronType == G4SBS::kPi0)
    { // gamma n --> pi0 n
      Mpi = G4PionZero::PionZeroDefinition()->GetPDGMass();
      M_ni = neutron_mass_c2;
      M_nf = M_ni;
  ...
  if (fabs(costhetaCM) > 1.0)
    return false; // kinematically forbidden
...
G4double G4SBSEventGen::TriangleFunc(G4double a, G4double b, G4double c)
{ // utility function to aid in computation of CM angle from mandelstam variables
...
void G4SBSEventGen::SofferBound(G4double x, G4double Q2, vector<double> &SofferBound_by_parton)
{
    ...
    for (int ix = 0; ix < nxbj; ix++)
    {
      for (int iQ = 0; iQ < nQ2; iQ++)
      {
        for (int iparton = 0; iparton < nparton; iparton++)
        {
          ...
          if (ix + 1 == nxbj)
            ftemp = 0.0; // because everything is assumed to go to zero at x = 1:
  ...
  while (logx < logxgrid[ibin_logx])
    ibin_logx--;
  // if logx is above the high edge of the bin, we guessed too low, increment:
  while (logx > logxgrid[ibin_logx + 1])
    ibin_logx++;
  // do the same for Q2:
  while (logQ2 < logQ2grid[ibin_logQ2])
    ibin_logQ2--;
  while (logQ2 > logQ2grid[ibin_logQ2 + 1])
    ibin_logQ2++;
...
void G4SBSEventGen::Transversity(G4double x, G4double Q2, vector<double> &h1_partons, int set)
{
  ...
  if (!fTransversityInitialized)
  {
    ...
    {
      gridfile >> fTran_a[iparton + 6 * iset] >> dummy >> fTran_b[iparton + 6 * iset] >> dummy >> fTran_n[iparton + 6 * iset] >> dummy >> fTran_m2[iparton + 6 * iset] >> dummy;
    ...
    for (iset = 1; iset <= 200; iset++)
    {
      for (iparton = 0; iparton < 6; iparton++)
      {
        gridfile >> fTran_a[iparton + 6 * iset] >> fTran_b[iparton + 6 * iset] >> fTran_n[iparton + 6 * iset] >> fTran_m2[iparton + 6 * iset];
...
void G4SBSEventGen::Sivers(G4double x, vector<double> &siv_partons, int set)
{
  ...
  if (!fSiversInitialized)
  {
    ...
    for (iparton = 0; iparton < 6; iparton++)
    {
      gridfile >> fSiv_a[iparton + 6 * iset] >> dummy >> fSiv_b[iparton + 6 * iset] >> dummy >> fSiv_n[iparton + 6 * iset] >> dummy >> fSiv_m2[iparton + 6 * iset] >> dummy;
    ...
    for (iset = 1; iset <= 200; iset++)
    {
      for (iparton = 0; iparton < 6; iparton++)
      {
        gridfile >> fSiv_a[iparton + 6 * iset] >> fSiv_b[iparton + 6 * iset] >> fSiv_n[iparton + 6 * iset] >> fSiv_m2[iparton + 6 * iset];
  ...
  if (siv_partons.size() < 6)
    siv_partons.resize(6);
...
void G4SBSEventGen::Collins(G4double z, vector<double> &coll_partons, int set)
{
  ...
  if (!fCollinsInitialized)
  {
    ...
    for (iparton = 0; iparton < 6; iparton++)
    {
      gridfile >> fColl_a[iparton + 6 * iset] >> dummy >> fColl_b[iparton + 6 * iset] >> dummy >> fColl_n[iparton + 6 * iset] >> dummy >> fColl_m2[iparton + 6 * iset] >> dummy;
    ...
    for (iset = 1; iset <= 200; iset++)
    {
      for (iparton = 0; iparton < 6; iparton++)
      {
        gridfile >> fColl_a[iparton + 6 * iset] >> fColl_b[iparton + 6 * iset] >> fColl_n[iparton + 6 * iset] >> fColl_m2[iparton + 6 * iset];
  ...
  if (coll_partons.size() != 6)
    coll_partons.resize(6);
...
double G4SBSEventGen::AUT_Sivers(G4double x, G4double y, G4double Q2, G4double z, G4double PT, vector<double> pdf_unpol, vector<double> fragfunc_unpol, G4SBS::Nucl_t nucleon, int iset)
{
  ...
  if (nucleon == G4SBS::kNeutron)
  { // swap distribution functions for quark density (but NOT fragmentation functions) between u and d:
...
double G4SBSEventGen::AUT_Collins(G4double x, G4double y, G4double Q2, G4double z, G4double PT, vector<double> pdf_unpol, vector<double> fragfunc_unpol, G4SBS::Nucl_t nucleon, G4SBS::Hadron_t hadron, int iset)
{
  ...
  if (nucleon == G4SBS::kNeutron)
  { // just swap the roles of u and d quark:
